...
description: Learn about table management in Citus, including how to determine table and relation size, work with distributed tables, and use columnar storage.
ms.date: 02/11/2026
...
Citus distributes tables across multiple nodes to achieve horizontal scaling. This article covers essential table management tasks specific to Citus, including measuring distributed table sizes, maintaining tables through vacuuming and analysis, and using columnar storage for analytical workloads. Understanding these operations helps you optimize performance and storage efficiency in your Citus cluster.

## Determining table and relation size

The usual way to find table sizes in PostgreSQL, `pg_total_relation_size`, drastically under-reports the size of distributed tables. On a Citus cluster, this function reveals the size of tables on the coordinator node. In reality the data in distributed tables lives on the worker nodes (in shards), not on the coordinator. A true measure of distributed table size is obtained as a sum of shard sizes. Citus provides helper functions to query this information.

These functions are analogous to three of the standard PostgreSQL [object size functions](https://www.postgresql.org/docs/current/static/functions-admin.html#FUNCTIONS-ADMIN-DBSIZE), with the added functionality that if they can't connect to a node, they error out.

Here's an example of using one of the helper functions to list the sizes of all distributed tables:
...
In PostgreSQL (and other multiversion concurrency control (MVCC) databases), an UPDATE, or DELETE of a row doesn't immediately remove the old version of the row. The accumulation of outdated rows is called bloat and must be cleaned to avoid decreased query performance and unbounded growth of disk space requirements. PostgreSQL runs a process called the autovacuum daemon that periodically vacuums (removes) outdated rows.

It's not just user queries which scale in a distributed database, vacuuming does too. In PostgreSQL large busy tables have great potential to bloat, both from lower sensitivity to PostgreSQL's vacuum scale factor parameter, and generally because of the extent of their row churn. Splitting a table into distributed shards means both that individual shards are smaller tables and that autovacuum workers can parallelize over different parts of the table on different machines. Ordinarily autovacuum can only run one worker per table.

Due to this consideration, autovacuum operations on a Citus cluster are probably good enough for most cases. However, for tables with particular workloads, or companies with certain "safe" hours to schedule a vacuum, it might make more sense to manually vacuum a table rather than leaving the work to autovacuum.

To vacuum a table, run this command on the coordinator node:

```sql
VACUUM my_distributed_table;
```

Using vacuum against a distributed table sends a vacuum command to every one of that table's placements (one connection per placement). This work is done in parallel. All [options](https://www.postgresql.org/docs/current/static/sql-vacuum.html) are supported (including the `column_list` parameter) except for `VERBOSE`. The vacuum command also runs on the coordinator, and does so before any workers nodes are notified. Unqualified vacuum commands (that is, commands without a table specified) don't propagate to worker nodes.

:::moniker range=">=citus-14"

> [!TIP]
> PostgreSQL 18 introduces `VACUUM/ANALYZE ONLY` to target only the parent table (for partitioned/inheritance hierarchies). Citus 14 honors this scoping when propagating maintenance commands.
>
> ```sql
...
> ```
>
> Use `ONLY` when you want a light-touch maintenance run without recursing into all child partitions or shard placements.

:::moniker-end

## Analyzing distributed tables

PostgreSQL's `ANALYZE` command collects statistics about the contents of tables in the database. The query planner uses these statistics to help determine the most efficient execution plans for queries.

The autovacuum daemon, discussed in the previous section, automatically runs `ANALYZE` commands whenever the content of a table changes sufficiently. The daemon schedules `ANALYZE` strictly as a function of the number of rows inserted or updated. It has no knowledge of whether that condition leads to meaningful statistical changes. Administrators might prefer to manually schedule `ANALYZE` operations instead, to coincide with statistically meaningful table changes.

To analyze a table, run this command on the coordinator node:
...
Citus converts rows to columnar storage in "stripes" during insertion. Each stripe holds one transaction's worth of data, or 150,000 rows, whichever is less. You can change the stripe size and other parameters of a columnar table by using the `alter_columnar_table_set` function.
...
Make large stripes when possible, because Citus compresses columnar data separately per stripe. You can see facts about your columnar table like compression rate, number of stripes, and average rows per stripe by using `VACUUM VERBOSE`:
...
Because of how the compression rate is measured, it might or might not match the size difference between row and columnar storage for a table. The only way to truly find that difference is to construct a row and columnar table that contain the same data, and compare.

### Measuring compression

To benchmark the compression savings, create a new example with more data.
...
For this data, you see a compression ratio of better than 8X in the columnar table.
...
Columnar storage works well with table partitioning. For an example, see `columnar_example`.
...
- On a partitioned table with a mix of row and column partitions, updates must be carefully targeted or filtered to hit only the row partitions.
  ...
  - If the operation is targeted at a specific row partition (for example, `UPDATE p2 SET i = i + 1`), it succeeds. If the operation is targeted at a specified columnar partition (for example, `UPDATE p1 SET i = i + 1`), it fails.
  - If the operation is targeted at the partitioned table and has a WHERE clause that excludes all columnar partitions (for example, `UPDATE parent SET i = i + 1 WHERE timestamp = '2020-03-15'`), it succeeds.
  - If the operation is targeted at the partitioned table, but doesn't exclude all columnar partitions, it fails. Even if the actual data to be updated only affects row tables (for example, `UPDATE parent SET i = i + 1 WHERE n = 300`).
