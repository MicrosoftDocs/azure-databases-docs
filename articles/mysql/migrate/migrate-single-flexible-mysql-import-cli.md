...
- Important considerations when using Virtual Network (VNet) and Azure Database for MySQL Import CLI :
      - **Avoid concurrent operations on VNet:** When you're executing an import operation, please refrain from performing any other operations on the same VNet. If there are other operations underway, please wait until they are fully completed before starting the import operation. This is to prevent any potential conflicts or resource contention.
      - **Limit simultaneous server migrations:** If you're planning to migrate multiple servers that reside under the same VNet, please do not initiate these migrations at the same time. Doing so could potentially cause the operations to be blocked in VNet, leading to extended wait times and even timeouts.
