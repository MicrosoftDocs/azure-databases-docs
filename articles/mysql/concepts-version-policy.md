...
After the standard support end date, Azure Database for MySQL servers running on unsupported MySQL versions are automatically enrolled in Extended Support and will be automatically charged for this service, after a one-month grace period. Extended Support provides critical security updates for up to three years after the end of standard support, allowing customers additional time to plan and execute major version upgrades. If you wish to avoid Extended Support charges, you must upgrade your Azure Database for MySQL server to a MySQL version that is still within standard support.
