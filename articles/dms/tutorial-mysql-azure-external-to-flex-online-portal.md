...
Azure DMS online migration supports migration to MySQL versions 5.7, 8.0, and 8.4. DMS also supports migration from lower version MySQL servers (v5.6 and later versions) to higher version servers. In addition, DMS supports cross-region, cross-resource group, and cross-subscription migrations, so you can select a region, resource group, and subscription for the target server that's different than what is specified for your source server.
