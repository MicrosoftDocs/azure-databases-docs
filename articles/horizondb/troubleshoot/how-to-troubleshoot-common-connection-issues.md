---
title: Troubleshoot Connections in Azure HorizonDB
description: Learn how to troubleshoot connection issues in Azure HorizonDB.
...
# Troubleshoot connection issues in Azure HorizonDB

Connection problems might be caused by various things, including:

- Firewall settings
- Connection time-out
- Incorrect login information
- Maximum limit reached on some Azure HorizonDB resources
...
Generally, connection issues to Azure HorizonDB can be classified as follows:
...
1. Applications that connect to a cloud service such as Azure HorizonDB should expect transient errors and implement retry logic to handle these errors instead of surfacing these as application errors to users. Review [Handling transient connectivity errors in Azure HorizonDB](concepts-connectivity.md) for best practices and design guidelines for handling transient errors.
1. As a server approaches its resource limits, errors can seem to be transient connectivity issue. See [Limits in Azure HorizonDB](../configure-maintain/concepts-limits.md).
...
If the application persistently fails to connect to Azure HorizonDB, it usually indicates an issue with one of the following:

- Server firewall configuration: Make sure that the Azure HorizonDB firewall is configured to allow connections from your client, including proxy servers and gateways.
...
1. On all firewalls between the client and the internet, make sure that port 5432 is open for outbound connections.
1. Verify your connection string and other connection settings.
1. Check the service health in the dashboard. If you think there's a regional outage, see [Overview of business continuity in Azure HorizonDB](../backup-restore/concepts-business-continuity.md) for steps to recover to a new region.

## Related content

- [Handling transient connectivity errors in Azure HorizonDB](concepts-connectivity.md)
