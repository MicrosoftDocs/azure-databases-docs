...
Autovacuum metrics can be used to monitor and tune autovacuum performance for Azure HorizonDB flexible server. Each metric is emitted at a *30-minute* interval and has up to *93 days* of retention. You can create alerts for specific metrics, and you can split and filter metrics data by using the `DatabaseName` dimension.
