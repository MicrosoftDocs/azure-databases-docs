---
title: Password Authentication Failed for User in Azure HorizonDB
description: Provides resolutions for a connection error - password authentication failed for user <user-name> in Azure HorizonDB.
...
# Password authentication failed for user `<user-name>` in Azure HorizonDB

This article helps you solve a problem that might occur when connecting to Azure HorizonDB.

## Symptoms

When attempting to connect to Azure HorizonDB, you might encounter the following error message:
...
The error encountered when connecting to Azure HorizonDB primarily stems from issues related to password authentication:
...
- **If you suspect a potential security breach**
  ...
  If you suspect a potential security breach is causing unauthorized access to your Azure HorizonDB, follow these steps to address the issue:
  ...
  1. **Enable log capturing**
     ...
     - **Log Analytics**, check out the setup instructions for Azure Monitor Log Analytics here: [Configure and access logs in Azure HorizonDB](../monitor/how-to-configure-and-access-logs.md).
     - **Server logs**, for hands-on log management, see [Download PostgreSQL and upgrade logs in Azure HorizonDB](../monitor/how-to-configure-server-logs.md).
