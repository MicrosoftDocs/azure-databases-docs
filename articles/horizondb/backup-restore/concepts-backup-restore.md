...
To enable geo-redundant backup from the **Compute + storage** pane in the Azure portal, see [Create an Azure HorizonDB](../configure-maintain/../configure-maintain/quickstart-create-server.md). 
...
Your Azure HorizonDB flexible server instance automatically generates storage volume snapshots of your entire database instance, covering all databases, as part of its scheduled backups. Additionally, you can create an on-demand backup whenever needed which is ideal for scenarios such as preparing for a potentially risky operation or performing periodic refreshes outside the usual backup schedule.
...
* **How do I manually back up my Azure HorizonDB?**
    ...
    You can manually take a physical snapshot using on-demand backup feature, you can also take logical backups using the PostgreSQL tool [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html). For examples, see {[Migrate your Azure HorizonDB database by using dump and restore](../howto-migrate-using-dump-and-restore.md)}. 
...
* **Are my backups encrypted?**
    ...
    Yes. All Azure HorizonDB flexible server instance data, backups, and temporary files that are created during query execution are encrypted through  AES (Advanced Encryption Standard) 256-bit encryption. Storage encryption is always on and can't be disabled. 
...
* **Where are my automated backups stored, and how do I manage their retention?**
    ...
    Your Azure HorizonDB flexible server instance automatically creates server backups and stores them in:
...
* **How are backups performed in a HA-enabled servers?**
    ...
    Data volumes in an Azure HorizonDB flexible server instance are backed up through managed disk incremental snapshots from the primary server. The WAL backup is performed from either the primary server or the standby server.
...
* **How will I be charged and billed for my backups?**
    ...
    Azure HorizonDB provides up to 100 percent of your provisioned server storage as backup storage at no extra cost. Any more backup storage that you use is charged in gigabytes per month, as defined in the pricing model. 
...
* **How do I restore my server?**
    ...
    To restore your server from manual backups by using tools like pg_dump, you can first create an Azure HorizonDB flexible server instance and then restore your databases to the server by using [pg_restore](https://www.postgresql.org/docs/current/app-pgrestore.html).
...
* **If I restore my HA-enabled server, is the restore server automatically configured with high availability?**
    ...
    No. The server is restored as a single-instance Azure HorizonDB flexible server instance. After the restore is complete, you can optionally configure the server with high availability.
