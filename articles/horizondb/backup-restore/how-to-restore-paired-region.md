---
title: Restore to Paired Region (Geo-Restore) in Azure HorizonDB
description: This article describes how to restore to paired region (geo-restore) in Azure HorizonDB.
...
ms.topic: how-to
# customer intent: As a user, I want to learn how to restore to paired region (geo-restore) in Azure HorizonDB.
---

# Restore to paired region (geo-restore) in Azure HorizonDB

This article provides step-by-step instructions to perform a restore in Azure HorizonDB to its paired region.
...
If the server whose backups you're trying to restore is configured with [Network with private access (virtual network integration) in Azure HorizonDB](../network/concepts-networking-private.md) networking mode, you can only restore to another virtual network in the remote region. You can restore your server into an existing virtual network or you can create a new virtual network.
...
1. Use the following table to understand the meaning of the different fields available in the **Compute + storage** page, and as guidance to fill the page:
   ...
   | **Storage** | | | | |
   | | **Storage type** | Select **Premium SSD**. | The list of allowed values might vary depending on which other features you selected. For more information, see {[Storage in Azure HorizonDB](../compute-storage/concepts-storage.md)}. | Can't be changed after the instance is created. |
   | | **Storage size** | Can't be changed and is automatically set to the same value as the source server. | The list of supported values might vary across regions, depending on the hardware available on each region. For more information, see [Compute options in Azure HorizonDB](../compute-storage/concepts-compute.md). | Can be changed after the instance is created. It can only be increased. Manual or automatic shrinking of storage isn't supported. Acceptable values depend on the type of storage assigned to the instance. |
...
- [Restore a dropped cluster in Azure HorizonDB](how-to-restore-dropped-server.md)
