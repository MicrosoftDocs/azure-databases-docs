...
description: This article describes how to restore to custom restore point an Azure HorizonDB flexible server.
...
9. Use the following table to understand the meaning of the different fields available in the **Compute + storage** page, and as guidance to fill the page:
    ...
    | **Storage** | | | | |
    | | **Storage type** | Select **Premium SSD**. | Notice that the list of allowed values might vary depending on which other features you selected. For more information, see {[Storage options in Azure HorizonDB flexible server](../extensions/concepts-storage.md)}. | Can't be changed after the instance is created. |
...
12. A new deployment is launched to create your new Azure HorizonDB flexible server and restore the most recent data available on the source server at the time of restore:
...
13. When the deployment completes, you can select **Go to resource**, to get you to the **Overview** page of your new Azure HorizonDB flexible server, and start using it:
