...
description: This article describes how to restore to custom restore point in Azure HorizonDB.
...
ms.topic: how-to
# customer intent: As a user, I want to learn how to restore to custom restore point in Azure HorizonDB.
---

# Restore to custom restore point in Azure HorizonDB

This article provides step-by-step instructions to perform a restore in Azure HorizonDB to a custom restore point.
...
> If you select a time which, when combined with the selected date, falls outside the period ranging from the earliest restore point and the time (UTC) at which you landed in the **Create Azure HorizonDB flexible Server - Restore server** wizard, and error asks you to adjust the value.
