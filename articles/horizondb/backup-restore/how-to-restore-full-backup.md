...
description: This article describes how to restore a full backup (fast restore) of an  Azure HorizonDB .
author: kabharati
ms.author: kabharati
ms.reviewer: maghan
ms.date: 04/22/2026
...
This article provides step-by-step instructions to perform a restore of an  Azure HorizonDB  to one of its available full backups.
...
5. In **Fast Restore point (UTC)**, select the timestamp of any of the full backups available to restore. The list includes the full backups that the service takes automatically.
...
1. Use the following table to understand the meaning of the different fields available in the **Compute + storage** page, and as guidance to fill the page:
   ...
   | | **Storage size** | Can't be changed and is automatically set to the same value as the source server. | Notice that the list of supported values might vary across regions, depending on the hardware available on each region. For more information, see [Compute options in  Azure HorizonDB ](../compute-storage/concepts-compute.md). | Can be changed after the instance is created. It can only be increased. Manual or automatic shrinking of storage isn't supported. Acceptable values depend on the type of storage assigned to the instance. |
    ...
    | | **Storage autogrow** | Can't be changed and is automatically set to the same value as the source server. | Notice that this option might not be supported for some storage types, and it might not be honored for certain storage sizes.  | Can be changed after the instance is created, as long as the storage type supports this feature. |
    | **Backups** | | | | |
...
Once you have the completion time of the full backup that you want to restore, you can restore the backup via the [az horizondb cluster restore](/cli/azure/postgres/flexible-server#az-postgres-flexible-server-restore) command.
