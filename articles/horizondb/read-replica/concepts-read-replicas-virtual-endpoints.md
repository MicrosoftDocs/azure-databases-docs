---
title: Virtual Endpoints in Azure HorizonDB
description: This article describes the virtual endpoints for read replica feature in Azure HorizonDB.
...
# Virtual endpoints for read replicas in Azure HorizonDB

Virtual Endpoints are read-write and read-only listener endpoints, that remain consistent irrespective of the current role of the Azure HorizonDB instance. This means you don't have to update your application's connection string after performing the **promote to primary server** action, as the endpoints will automatically point to the correct instance following a role change.
...
The sections below delve into how these endpoints react to both [Promote read replicas in Azure HorizonDB](concepts-read-replicas-promote.md) and **Promote to independent server** scenarios.
...
<a id="using-virtual-endpoints-for-consistent-hostname-during-point-in-time-recovery-pitr-or-snapshot-restore"></a>

### Use Virtual Endpoints for Consistent Hostname During Point-in-Time Recovery (PITR) or Snapshot Restore

This section explains how to use Virtual Endpoints in an Azure HorizonDB instance to maintain a consistent hostname during Point-in-Time Recovery (PITR) or Snapshot Restore, ensuring application connection strings remain unchanged. Follow below steps:

1. **Add Virtual Endpoint to Primary Server:**
   - Browse to your primary server instance in the Azure portal.
   - Navigate to **Replication** Tab, and under **Virtual Endpoints**, select **Add Virtual Endpoint**.
