...
description: Learn how to request a quota increase for Azure HorizonDB flexible server. You also learn how to enable a subscription to access a region.
...
# Request quota increases for Azure HorizonDB

The resources in Azure HorizonDB flexible server have default quotas/limits. However, there may be a case where your workload needs more quota than the default value. In such case, you must reach out to the Azure HorizonDB flexible server team to request a quota increase. This article explains how to request a quota increase for Azure HorizonDB flexible server resources. 

## Create a new support request

To request a quota increase, you must create a new support request with your workload details. The Azure HorizonDB flexible server team then processes your request and approves or denies it. Use the following steps to create a new support request from the Azure portal:
...
3. In the **Problem Description** tab, fill the following details:
   ...
   * For **Quota type**, select **Azure HorizonDB flexible server**

   :::image type="content" source="./media/how-to-create-support-request-quota-increase/create-quota-increase-request.png" alt-text="Create a new Azure HorizonDB flexible server request for quota increase.":::
...
The Azure HorizonDB flexible server support team processes all quota requests in 24-48 hours.

## Related content

- [Manage Azure HorizonDB flexible server using the Azure portal](how-to-manage-server-portal.md).
- [Limits in Azure HorizonDB flexible server](concepts-limits.md).
