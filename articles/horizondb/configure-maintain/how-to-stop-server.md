...
description: This article describes how to stop the compute of an Azure HorizonDB flexible server.
...
This article provides step-by-step instructions to stop the compute of an Azure HorizonDB flexible server instance.
...
> [!NOTE]
> Once the server is stopped, other management operations aren't available for the Azure HorizonDB flexible server.
> While the Azure HorizonDB flexible server is in stopped state, it could be briefly restarted for scheduled monthly maintenance, and then returned to its stopped state. This procedure ensures that even instances in a stopped state, stay up to date with all necessary patches and updates.
