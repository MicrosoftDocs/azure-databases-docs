...
description: This article describes how to restart the PostgreSQL engine of an Azure HorizonDB flexible server.
...
This article provides step-by-step instructions to restart the PostgreSQL engine hosted by an Azure HorizonDB flexible server.

> [!IMPORTANT]
> The restart of the PostgreSQL engine hosted in an Azure HorizonDB flexible server, doesn't restart the compute (virtual machine) which hosts it. It only restarts the database engine process.
...
> [!NOTE]
> Once the server is restarted, all management operations are available for the Azure HorizonDB flexible server.
