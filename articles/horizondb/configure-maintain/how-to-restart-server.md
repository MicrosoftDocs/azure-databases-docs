---
title: Restart PostgreSQL Engine in Azure HorizonDB
description: This article describes how to restart the PostgreSQL engine of an Azure HorizonDB.
...
ms.topic: how-to
# customer intent: As a user, I want to learn how to restart the PostgreSQL engine of an Azure HorizonDB.
---

# Restart PostgreSQL engine in Azure HorizonDB

This article provides step-by-step instructions to restart the PostgreSQL engine hosted by an Azure HorizonDB.

> [!IMPORTANT]  
> The restart of the PostgreSQL engine hosted in an Azure HorizonDB, doesn't restart the compute (virtual machine) which hosts it. It only restarts the database engine process.
...
1. When the process completes, a notification informs you that the server is restarted.
...
> [!NOTE]  
> Once the server is restarted, all management operations are available for the Azure HorizonDB.
