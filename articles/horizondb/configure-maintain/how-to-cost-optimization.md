---
title: How to Optimize Costs in Azure HorizonDB
description: This article provides a list of cost optimization recommendations in Azure HorizonDB.
...
# How to optimize costs in Azure HorizonDB

Azure HorizonDB service is a relational database service in the Microsoft cloud based on the [PostgreSQL Community Edition.](https://www.postgresql.org/). It's a fully managed database as a service offering that can handle mission-critical workloads with predictable performance and dynamic scalability.

This article provides a list of recommendations for optimizing Azure HorizonDB instance cost. The list includes design considerations, a configuration checklist, and recommended database settings to help you optimize your workload.
...
To learn more, see [Scale compute in Azure HorizonDB](../scale/how-to-scale-compute.md).
...
Key take away is to evaluate the requirement of your full application stack and then choose the right configuration for the Azure HorizonDB instance. For example, if your application isn't AZ resilient, there's nothing to be gained by configuring an Azure HorizonDB instance in AZ resilient configuration.

To learn more, see [High availability in Azure HorizonDB](/azure/reliability/reliability-postgresql-flexible-server).

## 5. Consolidate databases and servers

Consolidating databases can be a cost-saving strategy for Azure HorizonDB. Consolidating multiple databases into a single Azure HorizonDB instance can reduce the number of instances and overall cost of running Azure HorizonDB. Follow these steps to consolidate your databases and save costs:
...
1. Create a new Azure HorizonDB instance: Create a new Azure HorizonDB instance with enough vCPUs, memory, and storage to support the consolidated databases.
1. Reuse an existing Azure HorizonDB instance: In case you already have an existing server, make sure it has enough vCPUs, memory, and storage to support the consolidated databases.
1. Migrate the databases: Migrate the databases to the new Azure HorizonDB instance. You can use tools such as pg_dump and pg_restore to export and import databases.
1. Monitor performance: Monitor the performance of the consolidated Azure HorizonDB instance and adjust the resources as needed to ensure optimal performance.

Consolidating databases can help you save costs by reducing the number of Azure HorizonDB you need to run and by enabling you to use larger instances that are more cost-effective than smaller instances. It's important to evaluate the impact of consolidation on your databases' performance and ensure that the consolidated Azure HorizonDB instance is appropriately sized to meet all database needs.
...
Creating a test server in a cost-efficient Azure region can be a cost-saving strategy for Azure HorizonDB. By creating a test server in a region with lower cost of computing resources, you can reduce the cost of running your test server and minimize the cost of running Azure HorizonDB. Here are a few steps to help you create a test server in a cost-efficient Azure region:

1. Identify a cost-efficient region: Identify an Azure region with lower cost of computing resources.
1. Create a new Azure HorizonDB instance: Create a new Azure HorizonDB instance in the cost-efficient region with the right configuration for your test environment.
1. Migrate test data: Migrate the test data to the new Azure HorizonDB instance. You can use tools such as pg_dump and pg_restore to export and import databases.
...
By creating a test server in a cost-efficient Azure region, you can reduce the cost of running your test server and minimize the cost of running Azure HorizonDB. It's important to evaluate the impact of the region on your test server's performance and your organization's specific regional requirements. This ensures that network latency and data transfer costs are acceptable for your use case.

To learn more, see [Azure regions](/azure/architecture/framework/cost/design-regions)

## 7. Starting and stopping servers

Starting and stopping servers can be a cost-saving strategy for Azure HorizonDB. By only running the server when you need it, you can reduce the cost of running Azure HorizonDB. Here are a few steps to help you start and stop servers and save costs:

1. Identify the server: Identify the Azure HorizonDB instance that you want to start and stop.
1. Start the server: Start the Azure HorizonDB instance when you need it. You can start the server using the Azure portal, Azure CLI, or Azure REST API.
1. Stop the server: Stop the Azure HorizonDB instance when you don't need it. You can stop the server using the Azure portal, Azure CLI, or Azure REST API.
...
By starting and stopping the server as needed, you can reduce the cost of running an Azure HorizonDB instance. To ensure smooth database performance, it's crucial to evaluate the impact of starting and stopping the server and have a reliable process in place for these actions as required. To learn more, see [Stop compute of a server in Azure HorizonDB](how-to-stop-server.md) and [Start compute of a server in Azure HorizonDB](how-to-start-server.md).

## 8. Archive old data for cold storage

Archiving infrequently accessed data to Azure archive store (while still keeping access) can help reduce costs. Export data from your Azure HorizonDB instance to Azure Archived Storage and store it in a lower-cost storage tier.
...
To learn more, see [Migrate your PostgreSQL database to HorizonDB by using dump and restore](../migrate/how-to-migrate-using-dump-and-restore.md).

## Tradeoffs for cost

As you design your application database on Azure HorizonDB, consider tradeoffs between cost optimization and other aspects of the design, such as security, scalability, resilience, and operability.
