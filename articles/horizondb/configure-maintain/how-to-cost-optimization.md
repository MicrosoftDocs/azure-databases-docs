...
This article provides a list of recommendations for optimizing Azure HorizonDB flexible server instance cost. The list includes design considerations, a configuration checklist, and recommended database settings to help you optimize your workload.
...
Key take away is to evaluate the requirement of your full application stack and then choose the right configuration for the Azure HorizonDB flexible server instance. For example, if your application isn't AZ resilient, there's nothing to be gained by configuring an Azure HorizonDB flexible server instance in AZ resilient configuration.
...
Consolidating databases can be a cost-saving strategy for Azure HorizonDB. Consolidating multiple databases into a single Azure HorizonDB flexible server instance can reduce the number of instances and overall cost of running Azure HorizonDB. Follow these steps to consolidate your databases and save costs:
...
1. Create a new Azure HorizonDB flexible server instance: Create a new Azure HorizonDB flexible server instance with enough vCPUs, memory, and storage to support the consolidated databases.
1. Reuse an existing Azure HorizonDB flexible server instance: In case you already have an existing server, make sure it has enough vCPUs, memory, and storage to support the consolidated databases.
1. Migrate the databases: Migrate the databases to the new Azure HorizonDB flexible server instance. You can use tools such as pg_dump and pg_restore to export and import databases.
1. Monitor performance: Monitor the performance of the consolidated Azure HorizonDB flexible server instance and adjust the resources as needed to ensure optimal performance.

Consolidating databases can help you save costs by reducing the number of Azure HorizonDB you need to run and by enabling you to use larger instances that are more cost-effective than smaller instances. It is important to evaluate the impact of consolidation on your databases' performance and ensure that the consolidated Azure HorizonDB flexible server instance is appropriately sized to meet all database needs.
...
1. Create a new Azure HorizonDB flexible server instance: Create a new Azure HorizonDB flexible server instance in the cost-efficient region with the right configuration for your test environment.
1. Migrate test data: Migrate the test data to the new Azure HorizonDB flexible server instance. You can use tools such as pg_dump and pg_restore to export and import databases.
...
1. Identify the server: Identify the Azure HorizonDB flexible server instance that you want to start and stop.
1. Start the server: Start the Azure HorizonDB flexible server instance when you need it. You can start the server using the Azure portal, Azure CLI, or Azure REST API.
1. Stop the server: Stop the Azure HorizonDB flexible server instance when you don't need it. You can stop the server using the Azure portal, Azure CLI, or Azure REST API.
...
By starting and stopping the server as needed, you can reduce the cost of running an Azure HorizonDB flexible server instance. To ensure smooth database performance, it is crucial to evaluate the impact of starting and stopping the server and have a reliable process in place for these actions as required. To learn more, see [Stop compute of a server](how-to-stop-server.md) and [Start compute of a server](how-to-start-server.md).

## 8. Archive old data for cold storage

Archiving infrequently accessed data to Azure archive store (while still keeping access) can help reduce costs. Export data from your Azure HorizonDB flexible server instance to Azure Archived Storage and store it in a lower-cost storage tier. 
...
To learn more, see [Migrate your PostgreSQL database by using dump and restore](../migrate/how-to-migrate-using-dump-and-restore.md).
