...
1. In the Azure portal, select `pg_partman` in the list of Parameters for `azure.extensions`.
...
1. First, enable `pg_cron` on your server. In the Azure portal, add `pg_cron` to the `azure. extensions`, `shared_preload_libraries`, and `cron.database_name` Parameters.
   ...
   :::image type="content" source="media/how-to-use-pg-partman/pgcron-prerequisites.png" alt-text="Screenshot that shows adding pg_cron to the Parameter for Azure extensions." lightbox="media/how-to-use-pg-partman/pgcron-prerequisites.png" :::

   :::image type="content" source="media/how-to-use-pg-partman/pgcron-prerequisites-2.png" alt-text="Screenshot that shows adding pg_cron to the Parameter for shared preload libraries." lightbox="media/how-to-use-pg-partman/pgcron-prerequisites-2.png" :::
...
1. To schedule the `cron` job, use the following command:
...
1. To view all the `cron` jobs, use the following command:
...
1. To check the job's run history, use the following command:
