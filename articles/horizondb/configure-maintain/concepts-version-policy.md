...
You can continue to use the retired version in Azure HorizonDB. However, after the retirement date for each PostgreSQL database version, the following restrictions apply:

- When the community retires a PostgreSQL version, Azure HorizonDB stops applying bug or security patches to the database engine. This change might expose your server to security risks or other issues. However, Azure continues to maintain and patch the underlying host, operating system, containers, and related service components.
...
- New extensions introduced for Azure HorizonDB aren't supported on PostgreSQL versions that the community retired.
