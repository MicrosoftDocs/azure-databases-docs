...
description: Learn about compute replicas in Azure HorizonDB, including the primary and standby replica roles, scaling up vCores and memory, and scaling out by adding read replicas.
...
Because compute and storage are independent, you can scale compute resources without affecting storage, and vice versa. Azure HorizonDB supports two dimensions of compute scaling: scaling up (vertical) and scaling out (horizontal read scale out).

## Compute Replicas

Each compute replica runs a stateless PostgreSQL engine that connects to the shared storage layer. The replica processes queries, manages transactions, and maintains a local NVMe SSD cache for frequently accessed data pages. Compute replicas are provisioned with 8 GB of memory per vCore.As you increase the vCore count, memory scales proportionally. 
...
### Offloaded tasks
Because Azure HorizonDB uses a database-as-a-log architecture, compute replicas offload durability and high availability tasks to the storage layer. This offloading frees CPU, disk, and network resources on the compute replica to run application workloads. The following tasks are handled by the storage layer instead of the compute replica:
...
Scaling up increases the vCores and memory allocated to each compute replica in the cluster. Use scale up when your workload requires more processing power or memory per query, such as complex analytical queries, large joins, or high connection counts or needs more resources on your primary replica.
...
1. A new compute replica is provisioned distributed accross availability zones.
...
Because the storage is shared, provisioning a new replica is fast. There's no need to stream an initial data snapshot or catch up on a WAL backlog as in traditional PostgreSQL streaming replication.
...
- Removing a replica reduces the read capacity and the number of failover candidates. If the cluster has only one standby replica, removing it disables high availability.
...
Azure HorizonDB provides a dedicated read-only endpoint that automatically load balances connections across all standby replicas. Applications that separate read and write traffic can direct read queries to this endpoint to distribute load without any application-level routing logic.

The read-write endpoint always routes to the primary replica. Use it for all write operations and for reads that require the latest committed data.
