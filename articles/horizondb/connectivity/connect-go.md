---
title: "Quickstart: Use Go Language to Connect and Query Data in Azure HorizonDB"
description: This quickstart provides a Go programming language sample you can use to connect and query data from an Azure HorizonDB instance.
...
# Quickstart: Use Go language to connect and query data in Azure HorizonDB

This quickstart demonstrates how to connect to an Azure HorizonDB using code written in the [Go](https://go.dev/) language (golang). It shows how to use SQL statements to query, insert, update, and delete data in the database. This article assumes you're familiar with development using Go, but that you're new to working with Azure HorizonDB.
...
>
> Ensure the IP address you're connecting from has been added the server's firewall rules using the [Networking in Azure HorizonDB](../network/how-to-networking.md) or [Networking in Azure HorizonDB](../network/how-to-networking.md).
...
Get the connection information needed to connect to the Azure HorizonDB. You need the fully qualified server name and login credentials.
...
1. Locate the `HOST`, `DATABASE`, `USER`, and `PASSWORD` constants in the code, and replace the example values with your own values. A database named *postgres* is created when you create your Azure HorizonDB server instance. You can use that database or another one that you've created.
...
The code imports three packages: the [sql package](https://pkg.go.dev/database/sql), the [pq package](https://pkg.go.dev/github.com/lib/pq?utm_source=godoc) as a driver to communicate with the PostgreSQL server, and the [fmt package](https://pkg.go.dev/fmt) for printed input and output on the command line.

The code calls method [sql.Open()](https://pkg.go.dev/github.com/lib/pq?utm_source=godoc#Open) to connect to Azure HorizonDB database, and checks the connection using method [db.Ping()](https://pkg.go.dev/database/sql#DB.Ping). A [database handle](https://pkg.go.dev/database/sql#DB) is used throughout, holding the connection pool for the database server. The code calls the [Exec()](https://pkg.go.dev/database/sql#DB.Exec) method several times to run several SQL commands. Each time a custom checkError() method checks if an error occurred and panic to exit if an error does occur.
...
The code calls method [sql.Open()](https://pkg.go.dev/github.com/lib/pq?utm_source=godoc#Open) to connect to Azure HorizonDB database, and checks the connection using method [db.Ping()](https://pkg.go.dev/database/sql#DB.Ping). A [database handle](https://pkg.go.dev/database/sql#DB) is used throughout, holding the connection pool for the database server. The select query is run by calling method [db.Query()](https://pkg.go.dev/database/sql#DB.Query), and the resulting rows are kept in a variable of type  
[rows](https://pkg.go.dev/database/sql#Rows). The code reads the column data values in the current row using method [rows.Scan()](https://pkg.go.dev/database/sql#Rows.Scan) and loops over the rows using the iterator [rows.Next()](https://pkg.go.dev/database/sql#Rows.Next) until no more rows exist. Each row's column values are printed to the console out. Each time a custom checkError() method is used to check if an error occurred and panic to exit if an error does occur.
...
The code imports three packages: the [sql package](https://pkg.go.dev/database/sql), the [pq package](https://pkg.go.dev/github.com/lib/pq?utm_source=godoc) as a driver to communicate with the Postgres server, and the [fmt package](https://pkg.go.dev/fmt) for printed input and output on the command line.

The code calls method [sql.Open()](https://pkg.go.dev/github.com/lib/pq?utm_source=godoc#Open) to connect to Azure HorizonDB database, and checks the connection using method [db.Ping()](https://pkg.go.dev/database/sql#DB.Ping). A [database handle](https://pkg.go.dev/database/sql#DB) is used throughout, holding the connection pool for the database server. The code calls the [Exec()](https://pkg.go.dev/database/sql#DB.Exec) method to run the SQL statement that updates the table. A custom checkError() method is used to check if an error occurred and panic to exit if an error does occur.
