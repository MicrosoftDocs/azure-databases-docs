...
A **reader endpoint** for an Azure HorizonDB cluster connects to all replicas and provides automatic connection balancing. Each Azure HorizonDB has one reader endpoint that allows read-only operations such as SELECT queries. 

The reader endpoint automatically balances connections to all replicas in your Azure HorizonDB to scale out your read-only workloads such as anlyatical reporting.
