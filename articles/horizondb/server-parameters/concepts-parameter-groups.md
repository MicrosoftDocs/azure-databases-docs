...
- **Read-only parameters**: Certain parameters are marked as read-only by the system for security or stability reasons.
  - **Modification forbidden**: Any attempt to include a read-only parameter in a request will result in a validation error.
...
- **Scenario: Multi-cluster connection and deferred application**:
  ...
  - If a **static parameter** (e.g., `max_connections`) is updated in parameter group X and `applyImmediately` is set to `false`:
    ...
    1. If **cluster A** is manually restarted (or restarted due to maintenance), it will pick up the new static value and its status will transition to `InSync`.
    1. **cluster B**, if not restarted, will continue to run with the _previous_ value of the static parameter, even though its target configuration in the backend points to the new version. It will remain in a "Pending Restart" or "Out of Sync" state until a restart occurs.
...
| `properties.pgVersion` | integer | The PostgreSQL engine version (e.g., 17). |
