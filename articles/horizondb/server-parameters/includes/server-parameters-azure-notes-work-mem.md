...
description: work_mem server parameter for Azure HorizonDB.
...
<a id="monitoring-and-adjusting-work_mem"></a>

#### Monitor and adjusting work_mem
...
<a id="determining-the-minimum-work_mem-value-for-sorting-operations"></a>

##### Determine the minimum work_mem value for sorting operations
...
1. Set an initial `work_mem` value slightly higher than 20 MB to account for additional headers when processing in memory. Use a command such as: `SET work_mem TO '25MB'`.
1. Run `EXPLAIN ANALYZE` on the problematic query in the same session.
1. Review the output for `"Sort Method: quicksort Memory: xkB"`. If it indicates `"external merge Disk: xkB"`, raise the `work_mem` value incrementally and retest until `"quicksort Memory"` appears. The appearance of `"quicksort Memory"` signals that the query is now operating in memory.
1. After you determine the value through this method, you can apply it either globally or on more granular levels (as described earlier) to suit your operational needs.
