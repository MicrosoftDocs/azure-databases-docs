...
- **Storage layer** - Storage layer consists of purpose built storage fleets one for WAL, another for data both have durability backed by Azure storage. All storage is zone resilient by default. Storage scales automatically as data grows, independent of the provisioned compute tier. 
...
Azure HorizonDB adopts the database-as-a-log architecture where only WAL (Write ahead log) is the  written from Compute to the storage layer. Data pages are not written from the compute replicas to the storage layer. WAL is the authoritative source of truth through the system. 
...
## Cluster
A provisioned HorizonDB resource is a cluster. A HorizonDB cluster
- Has one or more compute replicas, one is a writable, the rest are readable.
- Has a single copy of zone resilient storage across all the replicas within region.
...
## Compute Replicas
A HorizonDB compute replica can be either the primary (writeable) or a secondary replica that is a readable  and also a candidate for failover. Compute replica is where the PostgreSQL relational engine lives and where the language, query, and transaction processing occurs. All interactions with the HorizonDB cluster happen through the compute replicas. In order to have zonal resilience, you need atleast two replicas on the cluster. You can add or remove replicas to the HorizonDB cluster as your workload needs it.


Compute replicas also have a local SSD cache. This cache is a low latency nvme cache that caches hot pages and minimizes the need to fetch data from the remote storage layer. The cache exists on all replicas primary and read replicas.
...
| Configurable backup retention | Not yet available | Currently backup retention is 7 days. We're working on enabling 1-35 days backup retention |
| Long-term retention (LTR) | Not available | Currently backup retention is 7 days. |
| Customer-managed keys (CMK) for encryption | Not available | Encryption at rest currently uses service-managed keys only. |
| Virtual network injection | Not available | Currently we support private link,  virtual network (vnet) integration isn't yet supported |
|Configurable maintenance windows | Not available | Currently upgrades occur on a system managed maintenance window. Ability to configure custom maintenance windows isn't yet available |
