...
description: This article describes the Query Performance Insight feature in an Azure HorizonDB flexible server instance.
...
3. **[Log analytics workspace](how-to-configure-and-access-logs.md)** is configured for storing 3 log categories including - Azure HorizonDB flexible server instance Sessions logs, Azure HorizonDB flexible server instance Query Store and Runtime, and Azure HorizonDB flexible server instance Query Store Wait Statistics. To configure log analytics, refer [Log analytics workspace](how-to-configure-and-access-logs.md#configure-diagnostic-settings).

> [!NOTE]
> The **Query Store data isn't being transmitted to the log analytics workspace**. The logs (Sessions data / Query Store Runtime / Query Store Wait Statistics) aren't being sent to the log analytics workspace, which is necessary to use Query Performance Insight. To configure the diagnostic settings for an Azure HorizonDB flexible server instance and send the data to a log analytics workspace, please refer to [Configure and access logs in Azure HorizonDB](how-to-configure-and-access-logs.md).
...
1. Open the Azure portal and find an Azure HorizonDB flexible server instance that you want to examine.
