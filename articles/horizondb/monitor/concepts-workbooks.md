---
title: Monitor by Using Azure Monitor Workbooks in Azure HorizonDB
description: This article describes how you can monitor an Azure HorizonDB instance by using Azure Monitor workbooks.
...
# Monitor by using Azure Monitor workbooks in Azure HorizonDB

Azure HorizonDB is integrated with Azure Monitor workbooks. Workbooks give you a flexible canvas for analyzing data and creating rich visual reports within the Azure portal. Workbooks allow you to tap into multiple data sources across Azure and combine them into unified interactive experiences. Workbook templates serve as curated reports designed for flexible reuse by various users and teams.
...
In this article, you learn about the various workbook templates available for your Azure HorizonDB instance.

An Azure HorizonDB instance has two available templates:
...
- **Enhanced metrics**: Displays a summary of enhanced metrics for your Azure HorizonDB instance, with more fine-grained database monitoring. To enable these metrics, enable the server parameters `metrics.collector_database_activity` and `metrics.autovacuum_diagnostics`. These parameters are dynamic and don't require a server restart. For more information, see [enhanced metrics](concepts-monitoring.md#enhanced-metrics) and [autovacuum metrics](concepts-monitoring.md#autovacuum-metrics). This template displays the following views:
