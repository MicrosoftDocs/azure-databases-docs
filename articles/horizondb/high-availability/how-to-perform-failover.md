...
3. Click **Manage replicas and failover**.
 ...
 4. Note the replicas assigned to **Primary endpoint (read/write)** and **Reader endpoint**. Additionally, note the **Zone** values assigned to  **ReadWrite** and **Read** Roles. 
...
6. After the operation completes, the replica becomes the primary server. You can validate and confirm that **Primary endpoint (read/write)**, **Reader endpoint**,  and **Zone** values assigned to  **ReadWrite** and **Read** Role are now reversed.
...
- [Business Continuity in Azure HorizonDB](../backup-restore/concepts-business-continuity.md)
