...
description: Providing the offline prerequisites for the migration service in Azure HorizonDB.
...
Before starting the migration with the Azure HorizonDB migration service, it is important to fulfill the following prerequisites, specifically designed for offline migration scenarios.
...
Before you begin the migration, you must set up an [Azure HorizonDB](/azure/postgresql/flexible-server/) in Azure.
...
To facilitate connectivity between the source and target PostgreSQL instances, it's essential to verify that networking is properly configured so that connectivity between source and target servers is allowed. When the source server is a PaaS service, like Amazon RDS for PostgreSQL or Google Cloud SQL for Postgres, it might require configuring some firewall rules and managed networking settings in them. When the source server is a sef-hosted server, it could require modifying the pg_hba.conf file of the source server. This file includes client authentication and must be configured to allow the target PostgreSQL to connect to the source. Changes to the pg_hba.conf file, typically require a restart of the source PostgreSQL instance to take effect.
...
- Match server parameter values from the source PostgreSQL database to the Azure HorizonDB by accessing the **Server parameters** page in the Azure portal, and manually updating the values accordingly.

- Save the parameter changes and restart the Azure HorizonDB to apply the new configuration, if necessary.
...
- **Manual migration of users and roles**: Users and roles must be manually migrated to the Azure HorizonDB. To facilitate this process, you can use the `pg_dumpall` utility with the `--globals-only` flag to export global objects, such as roles and users. Execute the following command, replacing `<<username>>` with the actual username, and `<<filename>>` with your desired output file name:

  ```sql
  pg_dumpall --globals-only -U <<username>> -f <<filename>>.sql
  ```

- **Restriction on superuser roles**: Azure HorizonDB doesn't support superuser roles. Therefore, users with superuser privileges must have those privileges removed before migration. Ensure that you adjust the permissions and roles accordingly.

By following these steps, you can ensure that user accounts and roles are correctly migrated to the Azure HorizonDB without encountering issues related to superuser restrictions.
