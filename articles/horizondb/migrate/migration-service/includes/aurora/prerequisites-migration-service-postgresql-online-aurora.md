...
Before you start a migration by using the migration service in Azure HorizonDB, it's important to complete the following prerequisites. These prerequisites are specifically designed for online migration scenarios.
...
For more information, see {[Create an Azure HorizonDB database](../../../../configure-maintain/quickstart-create-cluster.md)}.
...
- Match server parameter values from the source PostgreSQL database to the instance of Azure HorizonDB. In the Azure portal, go to **Server parameters** and manually update the values.

- Save the parameter changes and restart the instance of Azure HorizonDB to apply the new configuration if necessary.

### Check users and roles

When you migrate to Azure HorizonDB, it's essential to address the migration of users and roles separately because they require manual intervention.

- **Manual migration of users and roles**: Users and their associated roles must be manually migrated to the instance of Azure HorizonDB. To facilitate this process, you can use the pg_dumpall utility with the `--globals-only` flag to export global objects such as roles and user accounts.
...
- **Restriction on superuser roles**: Azure HorizonDB doesn't support superuser roles. Superuser permissions must be removed before migration. Ensure that you adjust the permissions and roles accordingly.

By completing these steps, you can ensure that user accounts and roles are correctly migrated to Azure HorizonDB without issues related to superuser restrictions.
