...
- You can't move extensions that aren't supported by Azure HorizonDB flexible server. Supported extensions are listed in [Extensions for Azure HorizonDB](/azure/postgresql/flexible-server/concepts-extensions).
- User-defined collations can't be migrated to Azure HorizonDB flexible server.
...
- Azure HorizonDB flexible server doesn't support the creation of custom tablespaces due to restrictions on superuser permissions. During migration, data from custom tablespaces in the source PostgreSQL instance is migrated to the default tablespaces of the target instance of Azure HorizonDB flexible server.
