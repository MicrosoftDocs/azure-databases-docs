---
title: Required Permissions for Migration Creation and Monitoring in Azure HorizonDB
description: Permissions required for a user to create and monitor migrations in Azure HorizonDB.
...
# Required permissions for migration creation and monitoring in Azure HorizonDB
...
<a id="assigning-a-custom-role-for-migration"></a>

### Assign a custom role for migration
...
For all the databases that you're planning to migrate - `CREATE` gives the user permission to create a publication to streams changes.
...
1. **Use Superuser credentials at the source for migration**: Superuser credential helps source user create the desired publication without any permission issues.
...
1. **Grant Create on Database + Transfer Ownership (Altering Owner)**: Allow a non-superuser migration user to create publications by giving them the needed privileges and control over tables.
...
Ownership of **all** tables of the Database to be migrated has to be changed to migration_user. Changing table ownership and reverting back might affect application permissions.
