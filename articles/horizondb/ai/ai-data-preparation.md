...
**Chunk size** is one of the highest-leverage tuning parameters in a search pipeline:
...
Adding **overlap** between consecutive chunks (repeating a portion of text at each boundary) helps preserve context across splits. A sentence straddling two chunks appears in both, preventing meaning from being lost at the boundary.
...
Without metadata, a semantic search returns the most similar chunks but can't restrict results to, for example, "only HR policy documents from 2024." Metadata also enables citation tracking, knowing which source document a chunk came from when presenting results to users or an LLM.
...
After cleaning, chunking, embedding, and enriching your data, store the results in a table and build the indexes needed for your search strategy.
...
Azure HorizonDB supports three vector index types (IVFFlat, HNSW, and DiskANN) each optimized for different scale and performance characteristics. For guidance on choosing the right index, see [Choose a vector index](vector-index-selection-guide.md).

### Full-text index

For hybrid search (combining keyword and vector results), also create a full-text search index on the chunk text using the `pg_fts` extension:

```sql
CREATE INDEX ON document_chunks USING fts (chunk_text);
```

pg_fts enables BM25-ranked keyword matching alongside vector similarity, which consistently improves retrieval quality over either approach alone. To learn more, see [Full-text search with pg_fts](full-text-search-pgfts.md).
...
- **Knowledge graph construction**: Extract entities and relationships from text and store them as a graph using Apache AGE, enabling relationship-based queries that complement vector search. For a walkthrough, see [Build a knowledge graph from unstructured text](build-knowledge-graph.md).
...
- [Search and retrieval overview](ai-search-overview.md)
...
- [Build a knowledge graph from unstructured text](build-knowledge-graph.md)
