...
1. **Fewer than 100,000 vectors and latency is not critical?** Use a flat (no index) scan. You get exact results with no tuning.
2. **More than 16,000 dimensions, or need to combine vector similarity with metadata filters at high recall?** Use **DiskANN**. It is the only option that supports both, through advanced filtering on HorizonDB.
3. **Dataset will exceed RAM, or rows added continuously, or expected to grow past ~10M?** Use **DiskANN**. It is designed to stay fast when most vectors live on SSD.
4. **Dataset is mostly static, fits comfortably in RAM, and recall must be high?** Use **HNSW**.
5. **Dataset is mostly static, fits in RAM, and you can tolerate lower recall in exchange for very fast builds?** Use **IVFFlat**.
6. **Anything else?** Default to **DiskANN**. It is the recommended index for new HorizonDB workloads at scale.
...
Use **DiskANN**. HNSW and IVFFlat are not viable at this dimensionality. See [Scalable vector indexing with DiskANN](vector-indexing-diskann.md) for the exact parameters.
