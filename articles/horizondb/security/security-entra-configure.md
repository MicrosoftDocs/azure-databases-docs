...
Use the Azure CLI to get an access token for the Microsoft Entra authenticated user to access Azure HorizonDB. Here's an example of the public cloud:
...
The token is a Base64 string. It encodes all the information about the authenticated user and is targeted to the Azure HorizonDB service.
...
Now connect to Azure HorizonDB:
...
- The access token is valid for 5 to 60 minutes. Get the access token before initiating the sign-in to Azure HorizonDB.

You're now authenticated to your Azure HorizonDB server through Microsoft Entra authentication.
