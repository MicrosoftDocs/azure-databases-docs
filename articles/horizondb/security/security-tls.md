...
# Transport Layer Security (TLS) in Azure HorizonDB
...
- Use client-side `sslmode` settings `disable`, `allow`, `prefer`, or `require` that can make your app vulnerable to man-in-the-middle attacks.
...
Azure HorizonDB supports TLS versions 1.2 and 1.3. In RFC 8996, the Internet Engineering Task Force (IETF) explicitly states that TLS 1.0 and TLS 1.1 must not be used. Both protocols were deprecated by the end of 2019.
...
### TLS features not available in Azure HorizonDB
