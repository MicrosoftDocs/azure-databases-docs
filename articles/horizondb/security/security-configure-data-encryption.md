---
title: Configure Data Encryption in Azure HorizonDB
description: Learn how to configure data encryption in Azure HorizonDB.
...
# Configure data encryption in Azure HorizonDB

This article provides step-by-step instructions to configure data encryption in Azure HorizonDB.
...
To learn about data encryption in the context of Azure HorizonDB, see the [data encryption](security-data-encryption.md).
...
1. During provisioning of a new Azure HorizonDB instance, data encryption is configured in the **Security** tab. For **Data encryption key**, select the **Service-managed key** radio button.
...
1. During provisioning of a new Azure HorizonDB instance, data encryption is configured in the **Security** tab. For **Data encryption key**, select the **Customer-managed key** radio button.
...
1. Expand **Key vault** (or **Managed HSM**, if you selected that storage type), and select the instance where the data encryption key exists.
   ...
   > [!NOTE]  
   > When you expand the dropdown list box, it shows **No available items**. It takes a few seconds until it lists all the instances of key vault which are deployed in the same region as the server.
...
You must update references that your Azure HorizonDB instance has to a key:
- When the key stored in the key store is rotated, either manually or automatically, and your Azure HorizonDB instance is pointing to a specific version of the key. If you're pointing to a key, but not to a specific version of the key (that's when you have **Use automatic key version update** enabled), then the service will take care of automatically reference the most current version of the key, whenever they key is manually or automatically rotated.
- When you want to use the same or a different key stored in a different key store.

You must update the user assigned managed identities which are used by your Azure HorizonDB instance to access the encryption keys whenever you want to use a different identity.
...
1. To change the user assigned managed identity with which the server accesses the key store in which the key is kept, expand the **User assigned managed identity** dropdown list, and select any of the identities available.
   ...
   > [!NOTE]  
   > Identities shown in the combo-box are only the ones that your Azure HorizonDB instance was assigned.
...
1. If the user assigned managed identity that you want to use to access the data encryption key isn't assigned to your Azure HorizonDB instance, and it doesn't even exist as an Azure resource with its corresponding object in Microsoft Entra ID, you can create it by selecting **Create**.

   :::image type="content" source="media/security-configure-data-encryption/existing-server-create-new-managed-identity.png" alt-text="Screenshot that shows how to create a new user assigned managed identities in Azure and Microsoft Entra ID, automatically assign it to your Azure HorizonDB instance, and use it to access the data encryption key." lightbox="media/security-configure-data-encryption/existing-server-create-new-managed-identity.png":::

1. In the **Create User Assigned Managed Identity** panel, complete the details of the user assigned managed identity that you want to create, and automatically assign to your Azure HorizonDB instance to access the data encryption key.
...
1. If the user assigned managed identity that you want to use to access the data encryption key isn't assigned to your Azure HorizonDB instance, but it does exist as an Azure resource with its corresponding object in Microsoft Entra ID, you can assign it by selecting **Select**.
...
1. Select **Use automatic key version update**, if you prefer to let the service automatically update the reference to the most current version of the chosen key, whenever the current version is rotated manually or automatically. To understand the benefits of using automatic key version updates, see [automatic key version update](security-data-encryption.md) key version updates).
...
1. If you rotate the key and don't have **Use automatic key version update** enabled. Or if you want to use a different key, you must update your Azure HorizonDB instance, so that it points to the new key version or new key. To do that, you can copy the resource identifier of the key, and paste it in the **Key identifier** box.
...
Identities passed to the `--identity` and `--backup-identity` parameters, if they exist and are valid, are automatically added to the list of user assigned managed identities associated to your Azure HorizonDB instance. This is the case even if the command later fails with some other error. In such cases, you might want to use the [az postgres flexible-server identity](/cli/azure/postgres/flexible-server/identity) commands to list, assign, or remove user assigned managed identities assigned to your Azure HorizonDB instance. To learn more about configuring user assigned managed identities in your Azure HorizonDB instance, refer to [associate user assigned managed identities to existing servers](security-configure-managed-identities-system-assigned.md), [dissociate user assigned managed identities to existing servers](security-configure-managed-identities-system-assigned.md), and [show the associated user assigned managed identities](security-configure-managed-identities-system-assigned.md).
