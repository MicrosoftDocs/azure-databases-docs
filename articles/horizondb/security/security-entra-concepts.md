---
title: Microsoft Entra Authentication in Azure HorizonDB
description: Learn about the concepts of Microsoft Entra ID for authentication in Azure HorizonDB
...
# Microsoft Entra authentication in Azure HorizonDB

Microsoft Entra authentication is a mechanism for connecting to Azure HorizonDB by using identities defined in Microsoft Entra ID. With Microsoft Entra authentication, you can manage database user identities and other Microsoft services in a central location, which simplifies permission management.
...
- Support of token-based authentication for applications that connect to Azure HorizonDB instance.

## How Microsoft Entra ID works in Azure HorizonDB

The following high-level diagram summarizes how authentication works when you use Microsoft Entra authentication with Azure HorizonDB. The arrows indicate communication pathways.
...
For the steps to configure Microsoft Entra ID with Azure HorizonDB, see [How to use Microsoft Entra ID for authentication in Azure HorizonDB](security-entra-configure.md).
...
The Microsoft Entra administrator can be a Microsoft Entra user, Microsoft Entra group, service principal, or managed identity. Using a group account as an administrator enhances manageability. It permits the centralized addition and removal of group members in Microsoft Entra ID without changing the users or permissions within the Azure HorizonDB instance.

You can configure multiple Microsoft Entra administrators concurrently. You can deactivate password authentication to an Azure HorizonDB instance for enhanced auditing and compliance requirements.

:::image type="content" source="media/security-entra-concepts/admin-structure.png" alt-text="Screenshot of admin structure for Entra ID." lightbox="media/security-entra-concepts/admin-structure.png" :::
...
- [Connect with managed identity in Azure HorizonDB](security-connect-with-managed-identity.md)
...
When you use Microsoft Entra authentication with Azure HorizonDB, keep the following points in mind:
...
- You can configure multiple Microsoft Entra principals (user, group, service principal, or managed identity) as Microsoft Entra administrators for an Azure HorizonDB instance at any time.

- Only a Microsoft Entra administrator for PostgreSQL can initially connect to the Azure HorizonDB instance by using a Microsoft Entra account. The Active Directory administrator can configure subsequent Microsoft Entra database users.
...
- If you delete a Microsoft Entra principal from Microsoft Entra ID, the principal remains as a PostgreSQL role but can no longer acquire a new access token. In this case, although the matching role still exists in the database, it can't authenticate to the server. Database administrators need to transfer ownership and drop roles manually.
  ...
  > [!NOTE]  
  > The deleted Microsoft Entra user can still sign in until the token expires (up to 60 minutes from token issuing). If you also remove the user from Azure HorizonDB, this access is revoked immediately.

- Azure HorizonDB matches access tokens to the database role by using the user's unique Microsoft Entra user ID, as opposed to using the username. If you delete a Microsoft Entra user and create a new user with the same name, Azure HorizonDB considers that a different user. Therefore, if you delete a user from Microsoft Entra ID and add a new user with the same name, the new user can't connect with the existing role.

## Frequently asked questions

- **What are the available authentication modes in Azure HorizonDB ?**

  Azure HorizonDB supports three modes of authentication: PostgreSQL authentication only, Microsoft Entra authentication only, and both PostgreSQL and Microsoft Entra authentication.
...
- **What happens when I enable Microsoft Entra authentication on my Azure HorizonDB instance?**
...
- **How do I generate my token?**
  ...
  You generate the token by using `az login`. For more information, see [How to use Microsoft Entra ID for authentication in Azure HorizonDB](security-entra-configure.md).
...
- [How to use Microsoft Entra ID for authentication in Azure HorizonDB](security-entra-configure.md)
