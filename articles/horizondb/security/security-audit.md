...
# Audit logging in Azure HorizonDB

Audit logging of database activities in Azure HorizonDB is available through the [`pgaudit`](https://www.pgaudit.org/) extension. `pgaudit` provides detailed session and/or object audit logging.
...
In Azure HorizonDB, `pgaudit.log` can't be set using a `-` (minus) sign shortcut as described in the `pgaudit` documentation. All required statement classes (`READ`, WRITE, etc.) should be individually specified.
