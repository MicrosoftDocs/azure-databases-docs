...
- **Rotate access keys regularly if using key-based authentication**: If your account has not yet migrated to role-based access control, rotate your primary and secondary keys on a regular schedule. Use the secondary key during primary key rotation to avoid downtime. For key rotation steps, see [Rotate account keys](how-to-rotate-keys.md). For migration to keyless authentication, see [role-based access control](how-to-connect-role-based-access-control.md).
