...
* **Authentication with the analytical store** - Supported authentication methods vary based upon whether networking features are enabled.
  ...
  - *Service Principal or Managed-Identity*: Using Entra Id or managed-identity authentication is only supported for accounts which do **not** use Private Endpoints or enable Vnet access. To use this type of authentication, users must apply [data plane RBAC](how-to-grant-data-plane-access.md) and create a new read only role with these data actions below.
