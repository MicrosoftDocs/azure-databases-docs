...
1. Review the job configuration and provide a unique job name.
    ...
    > 2. The migration job doesn't support changing the shard key. If you need a different shard key, migrate the data as an unsharded collection. Once the migration is complete, shard the collection on vCore using the desired shard key.
