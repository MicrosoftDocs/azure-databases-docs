...
description: The $topN operator in Azure Cosmos DB for MongoDB vCore returns the first N documents from the result sorted by one or more fields 
author: abinav2307
ms.author: abramees
...
The `$topN` operator sorts documents on one more fields specified by the query and returns the first N documents matching the filtering criteria.

## Syntax

```mongodb
{
  "$topN": {"output": [listOfFields], "sortBy": {"<fieldName>": <sortOrder>}, "n": <numDocumentsToReturn>}
}
```
...
### Example 1 - Retrieve the first 2 documents in the result set after sorting stores under the Boulder Innovations company in ascending order of total sales 

```mongodb
db.stores.aggregate([{"$match": {"company": { "$in": ["Boulder Innovations"] } } }, {"$group": {"_id": "$company", "topSales": {"$topN": {"output": ["$company", "$sales"], "sortBy": {"sales.totalSales": 1}, "n": 2} } } }])
```
...
### Example 2 - Retrieve the first 2 documents in the result set after sorting stores under the Boulder Innovations company in descending order of total sales 
