...
The same general principles from the [Azure Cosmos DB partitioning mechanism](../partitioning.md) apply with a few graph-specific optimizations described below.
...
- **Choose a partition key that will evenly distribute data across partitions**. This decision heavily depends on the data model of the solution. Read more about creating an appropriate partition key in [Partitioning and scale in Azure Cosmos DB](../partitioning.md).
