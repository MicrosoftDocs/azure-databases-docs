...
The partial document update feature improves this experience significantly. The client can only send the modified properties/fields in a document without doing a full document replace operation. Key benefits of this feature include:
...
| **Set**        | `Set` is similar to `Add` except with the array data type. If the target path is a valid array index, the existing element at that index is updated.   |
| **Replace**    | `Replace` is similar to `Set` except it follows _strict_ replace-only semantics. In case the target path specifies an element or an array that doesn't exist, it results in an error.   |
...
| **Move**       | This operator removes the value at a specified location and adds it to the target location. The operation object MUST contain a "from" member, which is a string containing a JSON Pointer value that references the location in the target document to move the value from. The "from" location MUST exist for the operation to be successful. If the "path" location suggests an object that doesn't exist, it creates the object and sets the value equal to the value at "from" location.<br/> &bull;If the "path" location suggests an object that already exists, it replaces the value at "path" location with the value at "from" location<br/> &bull;The "path" attribute can't be a JSON child of the "from" JSON location.   |

## Supported modes

Partial document update feature supports the following modes of operation. Refer to the [Get started](partial-document-update-getting-started.md) document for code examples.
...
- **Conditional update**: For the previously mentioned modes, it's also possible to add a SQL-like filter predicate (for example, `from c where c.taskNum = 3`) such that the operation fails if the precondition specified in the predicate isn't satisfied.
...
The `Set` operation is similar to `Add` for all data types except `Array`. An `Add` operation at any (valid) index, results in the addition of an element at the specified index and any existing elements in array end up shifting after the existing element. This behavior is in contrast to `Set` operation that updates the existing element at the specified index.

### Add vs Replace

The `Add` operation adds a property if it doesn't already exist (including `Array` data type). `Replace` operation fails if the property doesn't exist (applies to `Array` data type as well).

### Set vs Replace

The `Set` operation adds a property if it doesn't already exist (except if there was an `Array`). `Replace` operation fails if the property doesn't exist (applies to `Array` data type as well).
...
:::image type="content" source="./media/partial-document-update/patch-multi-region-conflict-resolution.png" alt-text="Diagram that shows conflict resolution in concurrent multi-region partial update operations." lightbox="./media/partial-document-update/patch-multi-region-conflict-resolution.png":::
...
The client sees the following document after conflict resolution:
