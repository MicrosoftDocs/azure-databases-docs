...
| Autoscaled RU | Shows the dynamically scaled provisioned throughput at each partition and region level only for dynamic autoscale enabled accounts. | Use this metric to see how partitions in each region scale independently based on their usage. <br/><br/> Use [Azure Monitor metrics](monitor-reference.md#supported-metrics-for-microsoftdocumentdbdatabaseaccounts) - `Autoscaled RU` to analyze how the new autoscaling is applied across partitions and regions. Filter to your desired database account and container, then filter or split by the Physical PartitionID metric. This metric shows all partitions across their various regions. |
    ...
    > [!IMPORTANT]
    > The throughput returned by the ReadThroughputAsync() API in the dataplane SDK and Provisioned Throughput Azure monitor metric present the 'Provisioned Throughput' which are indicative of billed throughput for that hour. This is updated with a delay of 5 to 15 minutes and the calculation for "Provisioned Throughput" may undergo implementation change as Cosmos DB continues to optimize the autoscale model. Users should not rely on these for making scale-up or scale-down decisions, as it does not reflect the normalized throughput consumption and can lead to throttling of workloads. To get an accurate view of whether a container can be safely scaled up or down,  Normalized RU Consumption metric in Azure Monitor need to be used.
