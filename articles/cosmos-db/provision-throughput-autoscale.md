...
> [!IMPORTANT]
>It is recommended to use Azure Cosmos DB's native dynamic scaling capability to manage your capacity. However, if needed, the [Normalized RU Consumption metric](https://learn.microsoft.com/en-us/azure/cosmos-db/monitor-normalized-request-units#metric-definition) in Azure Monitor can be used to make programmatic scaling decisions. Other approaches, like using the ReadThroughputAsync() call in the Azure Cosmos DB SDKs to get the ProvisionedThroughput, or using ProvisionedThroughput in Azure Monitor are not recommended and will lead to inaccurate results. These metrics represent billed throughput with a delay and shouldn't be used for scaling decisions.
