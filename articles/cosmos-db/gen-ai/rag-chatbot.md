...
At the end, we create a simple UX using Gradio to allow users to type in questions and display responses generated by Azure OpenAI or served from the cache. The responses also display an elapsed time to show the effect caching has on performance versus generating a response.

> [!TIP]
> For the full sample, see this [Python notebook](https://aka.ms/CosmosPythonRAGQuickstart).
>
