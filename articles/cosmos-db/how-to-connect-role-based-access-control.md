...
Azure Cosmos DB also provides built-in data plane roles (`Cosmos DB Built-in Data Reader` and `Cosmos DB Built-in Data Contributor`) to simplify common access patterns; see the full list of [built-in roles](https://learn.microsoft.com/en-us/azure/cosmos-db/reference-data-plane-security#built-in-roles)
