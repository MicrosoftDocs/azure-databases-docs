...
Azure Cosmos DB also provides built-in data plane roles (`Cosmos DB Built-in Data Reader` and `Cosmos DB Built-in Data Contributor`) to simplify common access patterns. For more information, see the full list of [built-in roles](/azure/cosmos-db/reference-data-plane-security#built-in-roles).
