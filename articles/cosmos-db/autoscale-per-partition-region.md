...
| Provisioned Throughput | Shows the aggregated highest RU/s scaled to across the hour, and represents the total RU/s scaled to for the hour. | You can use the `Provisioned Throughput` metric to see the RU/s you are billed for in each hour. With dynamic autoscale, you are billed for the aggregated highest RU/s scaled to in each hour at each partition and region level. |
| Normalized RU Consumption | This metric represents the ratio of consumed RU/s to provisioned RU/s at each partition and region level. |Use this metric to determine if the autoscale maximum throughput is under or over-provisioned. <br/><br/> If the metric value is consistently 100% and your application is seeing rate-limiting (429 error code), then this may be a sign you need more RU/s. In contrast, if this metric value is low and there is no rate-limiting, then there may be room to optimize and scale-down the RU/s. Learn how to [interpret and debug code 429 rate limiting errors](sql/troubleshoot-request-rate-too-large.md). <br/><br/> The `Normalized RU Consumption` metric reflects the RU/s consumed in secondary region due to write replication traffic from the primary, in addition to any read traffic on the secondary. |
