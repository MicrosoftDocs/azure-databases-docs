...
1. Create a new ``string`` variable named ``nosql`` with the query is used in this section to measure the distance between points.
    ...
    > [!TIP]
    > This query places the geospatial function within a [subquery](/nosql/query/subquery.md) to simplify the process of reusing the already calculated value multiple times in the [``SELECT``](query/select.md) and [``WHERE``](query/where) clauses.
...
The [LINQ to NoSQL](/nosql/query/linq-to-sql) functionality in the .NET SDK supports including geospatial types in the query expressions. Even further, the SDK includes extension methods that map to equivalent built-in functions:
