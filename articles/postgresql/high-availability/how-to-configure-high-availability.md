...
You can enable high availability in an existing server with the [az postgres flexible-server update](/cli/azure/postgres/flexible-server#az-postgres-flexible-server-update) command.

To enable high availability so that the standby server is in the same zone as the primary server, use this command:
...
To ensure your server is configured for high availability, enable the --allow-same-zone parameter. In regions without multiple availability zones, this option allows the standby server to be created in the same zone as the primary. When zonal capacity becomes available, Azure will notify you. At that point, you can use PITR or read replicas to migrate to a zone‑redundant HA configuration for maximum resiliency.  Refer to the [Limitations and Considerations](#limitations-and-considerations) section to know more.
...
1. Select the **Disabled** radio button to disable HA.
...
You can disable high availability in an existing server with the [az postgres flexible-server update](/cli/azure/postgres/flexible-server#az-postgres-flexible-server-update) command:
