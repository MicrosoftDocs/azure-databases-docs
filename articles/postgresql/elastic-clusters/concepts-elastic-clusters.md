---
title: Elastic Clusters in Azure Database for PostgreSQL Flexible Server
description: Learn about sharding and horizontal scale-out with elastic clusters on your Azure Database for PostgreSQL flexible server.
#customer intent: As a user, I want to understand how elastic clusters work, so that I can decide whether horizontal sharding fits my PostgreSQL workload.
author: JaredMSFT
...
While Citus is just an extension, it connects multiple PostgreSQL instances. When you deploy an Azure Database for PostgreSQL flexible server with Citus, it handles the management and configuration of multiple PostgreSQL instances as a single resource. It also automatically sets up the nodes and makes them known to the Citus extension.

Elastic clusters on the service offer two sharding models: row-based sharding and schema-based sharding. To learn more, see the open-source documentation about [sharding models](https://docs.citusdata.com/en/v12.1/get_started/concepts.html?highlight=shard#sharding-models).

## Architecture

An elastic cluster consists of one or more nodes of Azure Database for PostgreSQL flexible servers. These instances automatically discover each other and interconnect to form a Citus cluster. The nodes must be the same compute and storage tier, and you can scale them uniformly up or down to higher or lower tiers.
...
Elastic clusters use flexible servers (called nodes) to coordinate with one another in a **shared nothing** architecture. The architecture also allows the database to scale by adding more nodes to the cluster.
...
To learn more about Citus, see the official open-source [project documentation](https://docs.citusdata.com/).
...
Suppose that shard 102027 is associated with the row in question. The row is read or written in a table called `github_events_102027` in one of the workers. By using the information stored in the metadata tables, the extension determines which specific worker to use. The mapping of shard to worker is known as the shard placement.
