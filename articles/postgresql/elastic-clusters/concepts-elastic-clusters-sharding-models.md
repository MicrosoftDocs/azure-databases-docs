---
title: Sharding Models - Elastic Clusters in Azure Database for PostgreSQL Flexible Server
...
#customer intent: As a user evaluating scaling options, I want to understand the sharding models available in elastic clusters, so that I can choose the approach that best fits my application's requirements.
author: JaredMSFT
...
Elastic clusters on Azure Database for PostgreSQL flexible servers offer two types of data sharding: row-based and schema-based. Each option comes with its own [tradeoffs](#sharding-tradeoffs), so you can choose the approach that best aligns with your application's requirements.

## Row-based sharding

Shards tables in the single database shared schema model, also known as row-based sharding, tenants coexist as rows within the same table. You define a distribution column to determine the tenant, which splits up a table horizontally.

Row-based sharding is the most hardware-efficient method. The nodes in the cluster densely pack and distribute tenants. However, this approach requires making sure that all tables in the schema have the distribution column and that all queries in the application filter by that column. Row-based sharding works well in IoT workloads and for achieving the best margin out of hardware use.
...
| **Extra steps compared to vanilla PostgreSQL** | None, only a config change | Use `create_distributed_table` on each table to distribute and colocate tables by tenant ID |
