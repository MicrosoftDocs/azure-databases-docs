---
title: Manage Users in Azure Database for PostgreSQL Flexible Server
...
#customer intent: As a user, I want to create new user accounts in my Azure Database for PostgreSQL flexible server, so that I can control who has access to my databases.
author: Tameika-MSFT
ms.author: talawren
ms.reviewer: maghan
ms.date: 07/14/2026
...
This article describes how to create users within an Azure Database for PostgreSQL flexible server.
...
When you first create your Azure Database for PostgreSQL flexible server, you provide a server admin username and password. For more information, see [Create an Azure Database for PostgreSQL](../configure-maintain/quickstart-create-server.md) to see the step-by-step approach. Because the server admin user name is a custom name, you can locate the chosen server admin user name from the Azure portal.

The Azure Database for PostgreSQL flexible server is created with three default roles defined. You can see these roles by running the command: `SELECT rolname FROM pg_roles;`
...
Use the server admin user account to create more users and grant those users the azure_pg_admin role. Also, use the server admin account to create less privileged users and roles that have access to individual databases and schemas.

## How to create more admin users in Azure Database for PostgreSQL 

1. Get the connection information and admin user name.
You need the full server name and admin sign-in credentials to connect to your Azure Database for PostgreSQL flexible server. You can easily find the server name and sign-in information from the server **Overview** page or the **Properties** page in the Azure portal.

1. Use the admin account and password to connect to your Azure Database for PostgreSQL flexible server. Use your preferred client tool, such as pgAdmin or psql.
...
1. Edit and run the following SQL code. Replace the placeholder value `<db_user>` with your intended new user name and placeholder value `<newdb>` with your own database name. Replace the placeholder password with your own strong password.
    ...
    This SQL code creates a new database, then it creates a new user in the Azure Database for PostgreSQL flexible server and grants connect privilege to the new database for that user.
