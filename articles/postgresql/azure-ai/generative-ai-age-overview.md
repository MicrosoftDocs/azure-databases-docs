...
# AGE extension with Azure Database for PostgreSQL flexible server
...
> [!NOTE]  
> When AGE is selected in `shared_preload_libraries`, the library loads automatically when the server starts, so you don't need to run `LOAD 'age';`. Running it causes a privilege error.
