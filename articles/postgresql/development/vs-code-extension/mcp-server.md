...
# customer intent: As a user, I want to understand the PostgreSQL extension's MCP server registration and tools, so that AI-enabled hosts can discover database capabilities automatically.
---
...
1. The extension can retrieve a valid MCP server URL and bearer token from its language service.
1. Copilot integration is enabled.
...
> [!IMPORTANT]  
> The extension doesn't currently expose a manual copy-and-paste setup flow for standalone clients that expect you to enter the MCP URL and token yourself. If your client doesn't support host-level discovery or registration, use the extension's built-in AI workflows instead.
...
1. Save credentials where your workflow requires them.
1. Verify that you can connect to the target database from the extension before you rely on MCP-based automation.
...
- you want to chat directly with the [@pgsql chat participant](copilot/postgresql-chat-participant.md) (Visual Studio Code only),
- you want multi-step automation inside [Agent mode](copilot/agent-mode.md) (Visual Studio Code and Cursor), or
- your current host doesn't surface the extension's MCP registration automatically.

## Troubleshoot MCP availability

### The host doesn't show a PostgreSQL MCP server

Confirm that you're running a supported host and that Copilot/AI integration is enabled in the extension. If needed, reload the window after you sign in or change Copilot/AI settings.

### Registration is skipped

The extension skips registration when it can't retrieve a usable MCP URL or token from the language service. Check the extension output logs if the host never discovers the server after reload.

### A tool can't connect to my database
...
The extension doesn't currently provide a user-facing manual endpoint workflow for standalone MCP clients. Use a supported host or fall back to the built-in AI experiences documented in this doc set.
