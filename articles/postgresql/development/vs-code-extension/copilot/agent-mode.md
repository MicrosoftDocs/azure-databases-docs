...
# customer intent: As a user, I want to use Agent mode with PostgreSQL tools, so that my AI assistant can complete multi-step database workflows in my editor.
---
...
1. In the **Connections** tree, right-click the database you want to use.
1. Select **Connect AI**.
1. Continue the conversation in the chat panel that opens with the workflow you want to complete.
...
The following examples show how to use Agent mode for tasks that range from exploring a new database to refining queries.
...
Use [@pgsql chat participant](postgresql-chat-participant.md) when you want a schema-aware answer or a query draft. The `@pgsql` participant is available in **Visual Studio Code** only.
...
If Agent mode doesn't behave as expected, check access mode and tool availability first.

### The AI can't modify the database

Check `pgsql.copilot.accessMode` and confirm that the active connection isn't resolved to `ro`.
