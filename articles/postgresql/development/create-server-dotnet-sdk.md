---
title: "Quickstart: Create an Azure PostgreSQL Flexible Server by using the Azure SDK for .NET"
description: This document is a quickStart guide for Azure SDK library for .NET to create, update, and delete an Azure PostgreSQL flexible server.
#customer intent: As a .NET developer, I want to create an Azure Database for PostgreSQL flexible server using the Azure SDK, so that I can provision databases programmatically instead of using the portal.
author: gkasar
ms.author: gkasar
ms.reviewer: maghan
ms.date: 07/08/2026
...
- .NET framework installed on your local machine. Download it from [here](https://dotnet.microsoft.com/download).
...
Authenticate to your account by using [az CLI](/cli/azure/authenticate-azure-cli-interactively).
...
To learn more about the `.csproj` file, see [Web Deployment](/aspnet/web-forms/overview/deployment/web-deployment-in-the-enterprise/understanding-the-project-file).

> [!NOTE]  
> If you have problems related to initial setup for .NET, see [guide](/dotnet/core/install/windows).

## Create the project

Create a new .NET project by following the steps described in [link](/dotnet/core/tutorials/cli-templates-create-project-template).
