...
The query below will provide the bloat information on all the worker nodes. 
...
Here are the queries to check on percentage toward the wraparound on coordinator node and worker node:
...
Run the below query provides any uncommitted prepared transactions on worker nodes.
...
Run the below query provides any unused replication slots on worker nodes.
...
```
You cannot set the parameter values for a table shard.

The set value can be checked using the below query on the coordinator node:

```sql
SELECT relname, reloptions FROM pg_class WHERE relname = <table name>;
```

The query to check on all nodes:
