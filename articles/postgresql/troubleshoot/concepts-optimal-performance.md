...
But with all this power comes responsibility; most decisively when planning your deployments. To achieve the best possible performance, your deployment decisions must match your overall workload's requirements.
...
### Azure Postgres offers flexibility, but planning is essential
...
Azure storage performance planning usually comes down to three related but distinct concepts: IOPS, throughput, and latency are the key factors for application performance planning.
...
IOPS is especially important for OLTP workloads. These systems often perform many small, random reads and writes: inserts, updates, index lookups, point reads, and short transactions. A transactional workload with thousands of concurrent users may need high IOPS even if each individual operation is small.
...
**Throughput**, sometimes called bandwidth, measures how much data can be read from or written to storage over time, usually expressed in MB/s.

Throughput matters when operations move large amounts of data. Analytical queries, backups, restores, batch jobs, index builds, table scans, and ETL workflows may need high throughput even if they don't require the highest IOPS.
...
IOPS and throughput are connected. A workload issuing several small 8-KiB operations may drive high IOPS without high throughput. A workload issuing large multi MB operations may drive high throughput with lower IOPS.
...
A common mistake is to set your storage for a target performance number without fully considering whether your selected compute SKU is able to drive the same performance levels.
...
Azure storage performance has multiple considerations. These details include:
 ...
 1. The storage disk size (SSD v1 disks under 4,096 GB include host caching, allowing for IOPS bursts above standard baselines).
...
Premium SSD v1 has historically been a common choice for production Azure Postgres workloads requiring predictable, provisioned performance. Azure Postgres SSD v1 storage supports up to **32 TB** space, **20,000 IOPS**, and **900 MB/s** throughput.

Premium SSD v1 can be especially effective for workloads that benefit from host caching. Azure Postgres host caching is supported for SSD v1 disk sizes **less than 4,096 GB**. Any disk provisioned **up to 4,095 GB** can benefit from host caching. Once storage is provisioned at **4,096 GB or higher**, host caching isn't supported. **That boundary matters.**  For Premium SSD v1 deployments under 4 TB, caching can improve read performance and reduce read latency. This can create excellent cost-to-performance efficiency for read-heavy or mixed workloads that fit below the caching threshold.
...
Crossing above 4 TB isn't bad, but it must be *planned*.
...
This is a major shift. With Premium SSD v1, performance is more tightly coupled to disk size. With Premium SSD v2, teams can configure performance more directly around workload need.

For example, a transaction-heavy database may need high IOPS without needing a very large amount of storage. Premium SSD v2 makes that easier to model. Azure Postgres provides baseline IOPS and throughput at no extra cost, with additional IOPS and throughput available for additional charges. Premium SSD v2 offers:
...
Premium SSD v2 is often attractive when teams need more precise control over cost and performance. Instead of scaling storage capacity just to unlock performance, they can provision performance more intentionally.
 ...
 * Data Encryption with customer managed keys isn't supported for server with Ultra Disk
 * Ultra Disks don't support disk caching

Ultra Disk capabilities are important to understand as part of the broader Azure storage performance landscape, however, service availability and support must be validated for your specific Azure Postgres workload. Check with your Microsoft representative if the Ultra Disk Preview is available for your Azure Postgres deployment.
...
Compute generation can materially affect storage performance. When exploring the highest end of Azure storage performance, it's important to avoid the misunderstanding that "large compute" automatically means "maximum storage." The selected compute SKU must be validated against the intended storage tier. Let's illustrate this by considering two similarly sized compute generations, 'Ddsv5' and 'Ddsv6':

The Ddsv5-series support Premium Storage (w/ caching), Premium SSD v2, and Ultra Disk at the VM family level, but the VM's aggregate remote storage limits still define the ceiling for what that VM can drive. Ddsv5-series provide storage performance ranging up to **80,000 IOPS** and **2,600 MB/s**.

The Ddsv6-series provide a much higher storage envelope, ranging up to **400,000 IOPS** and **12,000 MB/s**. V6-series compute also offers higher scalability than prior generations, with up to 192 vCPU and 768-GiB memory.

That generational change is important for high-performance Postgres design. If your target architecture requires very high storage performance, choosing a compute generation with a lower aggregate storage ceiling can prevent the deployment from using the full storage capability.


### Example: why end-to-end alignment matters

Consider a Postgres workload with an aspirational storage target of 400,000 IOPS.

At the disk layer, Azure Ultra Disk can support up to 400,000 IOPS per disk. Premium SSD v2 supports up to 80,000 IOPS per disk, and higher aggregate designs may require multiple disks or platform-level abstraction depending on service support.

But storage capability alone isn't enough.

A V5-series configuration may have a storage ceiling that's lower than the target. As previously mentioned, V5-series SKUs support up to 260,000 IOPS for Premium SSD remote disk throughput. In this case, choosing the V5-series compute layer for this target becomes the limiting factor before a 400,000 IOPS target is reached.
...
The benchmarks used `pgbench`, the standard PostgreSQL benchmark tool, to simulate a transactional workload across five different storage and compute configurations. The test began with 500 concurrent connections and ramped up to 750 concurrent connections after an initial period, maintaining this elevated connection load for the remainder of the test window. This ramp-up pattern simulates how many real applications increase load over time as traffic grows, and measures how the database responds to both the initial spike and sustained high concurrency.

All benchmarks ran on Azure Database for PostgreSQL Flexible Server in the same region, within the same availability zone, using the same test database and workload profile, isolating storage and compute as the variables. This ensures that performance differences reflect actual platform capabilities rather than network, application, or workload variation.
...
- **Config 1 vs. Config 2:** These differ only in storage size, 4,095 GB vs. 4,096 GB. This comparison tests the host caching boundary for Premium SSD v1 disks.
- **Config 2 vs. Config 3:** Both use SSD v1, but Config 3 scales to 32-TB capacity to unlock higher IOPS and throughput.
- **Config 3 vs. Config 4:** Both use the same compute, but Config 4 demonstrates Premium SSD v2 flexible IOPS and throughput independent of capacity.
...
:::image type="content" source="./media/concepts-optimal-performance/4095.png" alt-text="Chart showing performance results for configuration 1 with 4,095-GB Premium SSD v1 storage and host caching." lightbox="./media/concepts-optimal-performance/4095.png":::

Configuration 1 uses the 4,095-GB SSD v1 size, which benefits from host caching on Premium SSD v1. During the workload, this configuration sustained:
...
:::image type="content" source="./media/concepts-optimal-performance/4096.png" alt-text="Chart showing performance results for configuration 2 with 4,096-GB Premium SSD v1 storage without host caching." lightbox="./media/concepts-optimal-performance/4096.png":::
...
:::image type="content" source="./media/concepts-optimal-performance/20000.png" alt-text="Chart showing performance results for configuration 3 with 32-TB Premium SSD v1 storage." lightbox="./media/concepts-optimal-performance/20000.png":::
...
Increasing the underlying Premium SSD v1 storage capacity increases IOPS and throughput. It's still possible to reach the upper limits of the compute storage range for intensive workloads.
...
Premium SSD v1 can provide strong predictable performance, especially when host caching applies below the 4-TB boundary. Premium SSD v2 adds more flexible performance configuration by decoupling capacity, IOPS, and throughput. Ultra Disk represents the highest Azure managed disk performance class, while newer compute generations provide substantially higher aggregate remote storage ceilings for high-end architectures.

The best Azure Postgres deployments combine platform capability with deliberate planning, continuous monitoring, and clear operational ownership. With the right requirements and the right architecture, teams can deliver world-class Postgres experiences providing peak performance.
