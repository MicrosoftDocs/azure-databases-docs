...
|---|---|---|---|
| Ask a schema-aware question about your database | **@pgsql** chat participant | Visual Studio Code only | [@pgsql chat participant](copilot/pgsql-chat.md) |
| Start Agent mode with a specific database already in scope | **Connect AI** from a database node in **Object Explorer** | Visual Studio Code and Cursor | [Agent mode](copilot/agent-mode.md) |
...
> [!NOTE]
> The **@pgsql** chat participant is registered only in **Visual Studio Code**, where it plugs into GitHub Copilot Chat. In **Cursor**, use **Connect AI** from a database node in **Object Explorer** to start Agent mode with the same schema context, or rely on the MCP server in Cursor's native chat surface.
...
To start a tool-driven workflow with a database already in scope, right-click a database node in **Object Explorer** and select **Connect AI**.

For end-user guidance, example prompts, and troubleshooting, continue to [@pgsql chat participant](copilot/pgsql-chat.md).
...
Agent mode gives the AI assistant access to PostgreSQL tools so it can connect, inspect schema, run queries, open scripts, and work through multi-step tasks in one conversation. Right-click a database node in **Object Explorer** and select **Connect AI** to start an Agent mode session.
