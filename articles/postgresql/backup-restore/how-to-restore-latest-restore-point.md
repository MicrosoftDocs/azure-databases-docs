...
ms.topic: how-to
#customer intent: As a user, I want to learn how to restore to latest restore point an Azure Database for PostgreSQL.
---
...
8. Use the following table to understand the meaning of the different fields available in the **Compute + storage** page, and as guidance to fill the page:
    ...
    | **Storage** | | | | |
    | | **Storage type** | Select **Premium SSD**. | Notice that the list of allowed values might vary depending on which other features you selected. For more information, see [Storage options in Azure Database for PostgreSQL flexible server](../extension-module/concepts-storage.md). | Can't be changed after the instance is created. |
