---
title: Restore to Latest Restore Point in Azure Database for PostgreSQL Flexible Server
description: Restore to latest restore point in Azure Database for PostgreSQL flexible server using the Azure portal or CLI. Follow these instructions to restore to latest restore point.
...
# Restore to latest restore point in Azure Database for PostgreSQL flexible server

This article provides step-by-step instructions for restoring an Azure Database for PostgreSQL flexible server to the latest restore point.
...
1. You're redirected to the **Create Azure Database for PostgreSQL flexible server - Restore server** wizard, where you can configure some settings for the new server. After the new server is deployed, the most recent snapshot of the source server data disk is restored. In the **Point-in-time-restore (PITR)** section, select **Latest restore point (Now)**.
...
1. When you configure the new server, select **Review + create**.
...
1. Review the configurations for the new deployment. When everything is correct, select **Create**.
...
1. When the deployment completes, select **Go to resource**. This selection takes you to the **Overview** page of your new Azure Database for PostgreSQL flexible server, and you can start using it:
...
> [!NOTE]
> The `az postgres flexible-server restore` command accepts a `--restore-time` parameter. Provide a value that represents the point in time, in UTC, to restore from (ISO8601 format). One example valid value is `2026-06-26T02:10:00+00:00`. If you don't specify this parameter, the value defaults to the current time in the system where you run the command.
