---
title: Restore Full Backup (Fast Restore) in Azure Database for PostgreSQL Flexible Server
description: Restore a full backup in Azure Database for PostgreSQL flexible server using the Azure portal or CLI. Follow these instructions to restore your data now.
...
# Restore full backup (fast restore) in Azure Database for PostgreSQL flexible server

This article provides step-by-step instructions to restore an Azure Database for PostgreSQL flexible server to one of its available full backups.
...
1. You're redirected to the **Create Azure Database for PostgreSQL flexible server - Restore server** wizard, where you can configure some settings for the new server. After the new server is deployed, the most recent snapshot of the source server data disk is restored. In the **Point-in-time-restore (PITR)** section, select **Fast restore point (Restore using full backup only)**.

    :::image type="content" source="./media/how-to-restore-server/fast-restore-point.png" alt-text="Screenshot showing the Fast restore point (Restore using full backup only) radio button selected." lightbox="./media/how-to-restore-server/fast-restore-point.png":::

1. In **Fast Restore point (UTC)**, select the timestamp of any of the full backups available to restore. The list includes the full backups that the service takes automatically, and any [on-demand backups](how-to-perform-backups.md) taken by the user.
...
10. When you finish configuring the new server, select **Review + create**.

    :::image type="content" source="./media/how-to-restore-server/restore-point-fast-review-create.png" alt-text="Screenshot showing the location of the Review + create button." lightbox="./media/how-to-restore-server/restore-point-fast-review-create.png":::

11. Review the configurations for the new deployment. When you're satisfied, select **Create**.

    :::image type="content" source="./media/how-to-restore-server/restore-point-fast-create.png" alt-text="Screenshot showing the location of the Create button." lightbox="./media/how-to-restore-server/restore-point-fast-create.png":::
...
13. When the deployment completes, select **Go to resource**. You go to the **Overview** page of your new Azure Database for PostgreSQL flexible server, and start using it:
...
Use the [az postgres flexible-server backup list](/cli/azure/postgres/flexible-server/backup#az-postgres-flexible-server-backup-list) command to find the backup you want to restore. Note the exact date and time when the backup completed.
...
Use the [az postgres flexible-server restore](/cli/azure/postgres/flexible-server#az-postgres-flexible-server-restore) command to restore the backup after you get the completion time for the full backup you want to restore.
