...
To perform a full table repack the extension follows these steps:
...
6.    Using the system catalogs, swaps the original and new tables, including indexes and toast tables.
7.    Drops the original table.

During these steps, `pg_repack` only holds an exclusive access lock for a short period, during the initial setup (steps 1 and 2) and again during the final swap-and-drop phase (steps 6 and 7). For the rest of the time, `pg_repack` will only need to hold a shared access lock on the original table, allowing INSERTs, UPDATEs, and DELETEs to proceed as usual.

## Limitations

`pg_repack` has some limitations that you should be aware of before using it:

-  The `pg_repack` extension can't be used to repack the default database named `postgres`. This is because `pg_repack` doesn't have the necessary permissions to operate against extensions installed by default on this database. The extension can be created in that database, but it can't run.
...
-  While `pg_repack` is running, you won't be able to perform any DDL commands on the target tables except for VACUUM or ANALYZE. To ensure these restrictions are enforced, `pg_repack` will hold an shared access lock on the target table during a full table repack.
...
Run the following bash script on an Ubuntu machine to install version 1.4.7 of `pg_repack`.
...
1. Using the client of your preference, connect to the Azure Database for PostgreSQL flexible server instance. In this example we use psql.
...
To understand all the options, refer to the documentaiton of [pg_repack](https://reorg.github.io/pg_repack/).
