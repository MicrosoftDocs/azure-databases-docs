...
description: Learn about the concepts of Geo-disaster recovery with an Azure Database for PostgreSQL flexible server instance.
author: jjfrost
ms.author: jfrost
ms.reviewer: maghan
ms.date: 04/27/2024
...
Cross region read replicas can be deployed to protect your databases from region-level failures. Read replicas are updated asynchronously using an Azure Database for PostgreSQL flexible server instance's physical replication technology, and can lag the primary. Read replicas are supported in general purpose and memory optimized compute tiers.
