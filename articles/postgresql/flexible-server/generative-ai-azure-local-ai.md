...
Before you can enable azure_local_ai on your Azure Database for PostgreSQL flexible server instance, you need to allowlist the `azure_local_ai` extension as described in [allow an extension](how-to-allow-extensions#allow-extensions).
...
Once the extension is allowlisted, you can follow the instructions provided in [create extensions](how-to-allow-extensions#create-extensions) to install the extension in each  database from where you want to use it.
