...
However, none of those two methods reveal the text of the queries for which the recommendations were produced. This is an intentional behavior, because the texts of the queries might contain sensitive information. Therefore seeing those statements should only be allowed to subjects with authorization to access the database, but not to subjects who have only been granted access to the instance of Azure Database for PostgreSQL flexible server, as an Azure resource.
...
The command returns all information about the CREATE INDEX recommendations produced by index tuning, and the output is similar to the following:
...
The command returns all information about the DROP INDEX recommendations produced by index tuning, and the output is similar to the following:
