...
12. Use the following table to understand the meaning of the different fields available in the **Virtual Network** page, and as guidance to fill the page:
    ...
    | **Private IP configuration** | Automatically set to dynamically allocate one of the available IP addresses in the range assigned to the selected subnet. | This is the IP address assigned to the network interface associated to the private endpoint. It can be dynamically allocated from the range assigned to the selected subnet, or you can decide which specific address you want to assign to it. After the private endpoint is created, you can't change its IP address, regardless of which of the two allocation modes you select during creation. |
