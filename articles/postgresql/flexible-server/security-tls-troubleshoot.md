...
Install either the minimum required root certificates or the full set of root certificates in your client's root certificate store.

> [!CAUTION]
> Trust only Azure root CA certificates in your clients' root certificate store. Avoid trusting intermediate CAs or individual server certificates. These practices can lead to unexpected connection problems when Microsoft updates the certificate chain or rotates individual server certificates.
...
When root CA migration occurs to [Microsoft RSA Root CA 2017](https://www.microsoft.com/pkiops/docs/repository.htm), newly created replicas can use a newer root CA certificate than the primary server that uses an older root CA certificate. For clients that use `sslmode=verify-ca` and `sslmode=verify-full` configuration settings, you need to accept both the new and previous root CA certificates until the rotation finishes on new and existing servers.
...
1. The newest certificate issued by [Microsoft RSA Root CA 2017](https://www.microsoft.com/pkiops/docs/repository.htm) has an intermediate certificate in the chain that's cross-signed by Digicert Global Root G2 CA. Some Postgres client libraries might experience connection failures with root CA certificates that are cross-signed with intermediate certificates when they use `sslmode=verify-full` or `sslmode=verify-ca` settings. The result is alternate trust paths.
...
1. List the certificates in your trusted root store.
...
1. List the certificates in your trusted root store. For example:
