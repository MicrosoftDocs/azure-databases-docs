---
title: Connect with the Private Access Connectivity Method in Azure Database for PostgreSQL Flexible Server
description: This article shows how to create and connect to an Azure Database for PostgreSQL flexible server with private access or virtual network using the Azure portal.
#customer intent: As a user, I want to create an Azure Database for PostgreSQL flexible server in a virtual network, so that I can run a highly available database with private access.
author: gkasar
ms.author: gkasar
ms.reviewer: maghan
ms.date: 07/08/2026
...
# Connect by using the private access connectivity method in Azure Database for PostgreSQL flexible server

Azure Database for PostgreSQL is a managed service that you can use to run, manage, and scale highly available PostgreSQL servers in the cloud. This article shows you how to create an Azure Database for PostgreSQL flexible server in a virtual network by using the Azure portal.
...
You create an Azure Database for PostgreSQL flexible server with a defined set of [compute and storage resources](../compute-storage/concepts-compute.md). You create the server within an [Azure resource group](/azure/azure-resource-manager/management/overview).

Complete these steps to create an Azure Database for PostgreSQL flexible server:
...
1. On the **Basics** tab, enter the **subscription**, **resource group**, **region**, and **server name**.  With the default values, this configuration provisions an Azure Database for PostgreSQL flexible server of version 12 with General purpose pricing tier  using 2 vCores, 8 GiB RAM, and 28 GiB storage. The backup retention is **seven** days. Use the **Development** workload to default to a lower-cost pricing tier.
...
1. In the **Basics** tab, enter  a unique **admin username** and **admin password**.
...
1.  Go to the **Networking** tab, and select **private access**. You can't change the connectivity method after you create the server. Select **Create virtual network** to create a new  virtual network **vnetenvironment1**. Select **OK** once you provide the virtual network name and subnet information.
...
1. Select **Review + create** to review your Azure Database for PostgreSQL flexible server configuration.
...
1.  Select **Go to resource** to view the server's **Overview** page.

## Create an Azure Linux virtual machine

Because the server is in a virtual network, you can only connect to the server from other Azure services in the same virtual network as the server. To connect and manage the server, create a Linux virtual machine. You must create the virtual machine in the **same region** and **same subscription**. Use the Linux virtual machine as an SSH tunnel to manage your Azure Database for PostgreSQL flexible server. 

1. Go to your resource group where you created the server. Select **Add**.
...
1. Under **Instance details**, type *myVM* for the **Virtual machine name**, and choose the same **Region** as your Azure Database for PostgreSQL flexible server.
...
1. For **SSH public key source**, keep the default of **Generate new key pair**, and then type *myKey* for the **Key pair name**.
...
1. Under **Inbound port rules** > **Public inbound ports**, choose **Allow selected ports**. Then select **SSH (22)** and **HTTP (80)** from the drop-down.
...
1. Select the **Networking** page to configure the virtual network. For the virtual network, choose the **vnetenvironment1** created for the database server.
...
1. Add the new subnet for the virtual machine.
...
1. After the subnet is created successfully, close the page.
...
1. Select **Create**. When the **Generate new key pair** window opens, select **Download private key and create resource**. Your key file is downloaded as **myKey.pem**.
    ...
    >[!IMPORTANT]
    > Make sure you know where the `.pem` file is downloaded. You need the path to it in the next step.

1. When the deployment finishes, select **Go to resource** to view the virtual machine **Overview** page.

1. Select the public IP address and copy it to your clipboard.
...
Create an SSH connection with the VM by using Bash or PowerShell. At your prompt, open an SSH connection to your virtual machine. Replace the IP address with the one from your VM, and replace the path to the `.pem` with the path to where the key file was downloaded.
...
> [!TIP]
> You can use the SSH key you created the next time you create a VM in Azure. Just select **Use a key stored in Azure** for **SSH public key source** the next time you create a VM. You already have the private key on your computer, so you don't need to download anything.

You need to install the `postgresql-client` tool to connect to the server.
...
Because connections to the database require SSL, you need to download the public SSL certificate.
...
With the **psql** client tool installed, you can now connect to the server from your local environment.
...
## Clean up resources
You created an Azure Database for PostgreSQL flexible server in a resource group. If you don't expect to need these resources in the future, you can delete them by deleting the resource group, or you can just delete the Azure Database for PostgreSQL flexible server. To delete the resource group, complete the following steps:
