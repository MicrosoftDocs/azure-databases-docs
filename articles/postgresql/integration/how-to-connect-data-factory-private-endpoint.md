...
#customer intent: As a user, I want to connect Azure Data Factory to an Azure Database for PostgreSQL flexible server using a private endpoint, so that I can move data securely over a private network.
...
- An Azure Database for PostgreSQL flexible server with its network connectivity method configured as **Public access (allowed IP addresses)** so that you can create [private endpoints](../flexible-server/../network/concepts-networking-private-link.md) to connect to it privately using Azure Private Link.
...
After you deploy a private endpoint, you must approve it to permit incoming traffic. Suppose you have access to Azure Data Factory and have permission to approve private endpoints created against the Azure Database for PostgreSQL flexible server. In that case, you can use the **Managed private endpoints** page of [Azure Data Factory Studio](https://adf.azure.com), select the name of the managed private endpoint, and, on the opening pane, select **Manage approvals in Azure portal**.
...
After you deploy and approve the private endpoint, use your data factory's Azure Database for PostgreSQL connector to create a linked service. You can connect to your Azure Database for PostgreSQL flexible server through that linked service.

1. In [Azure Data Factory Studio](https://adf.azure.com), select the **Manage** hub. Under the **Connections** section, select **Linked services**, and then select **New** to create a new linked service:
...
1. Fill in all required fields for the connector. Ensure that you select the integration runtime where you created the private endpoint in its managed virtual network. Also, make sure that the **Interactive authoring** feature is enabled on that integration runtime so that you can test the connection when all required information is provided.
...
1. Select an **Encryption method**. If you select **No encryption**, the connection only succeeds if the parameter [require_secure_transport](../parameters/parameters-tls.md?#require_secure_transport) is set to `off`. This setting isn't a recommended practice since it relaxes security.
...
1. Select **Test connection**. A **Connection successful** message appears next to the **Test connection** button.
