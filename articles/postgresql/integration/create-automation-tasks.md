---
title: Manage Azure Database for PostgreSQL flexible server using automation tasks
description: This article describes how to stop/start an Azure Database for PostgreSQL flexible server by using automation tasks.
#customer intent: As a user, I want to create an automation task that stops my Azure Database for PostgreSQL flexible server on a schedule, so that I can reduce costs during off-hours.
...
You can create automation tasks for your Azure Database for PostgreSQL flexible server to start or stop the server on a predefined schedule. Set the Interval and Frequency values on the task's Configure tab to automatically start or stop the server a specific number of times every day, week, or month. The automation task continues to work until you delete or disable the task.
...
- An Azure Database for PostgreSQL flexible server you want to manage.
...
1. Under **Select a template**, select the task for stopping your Azure Database for PostgreSQL flexible server.
...
The task you created, which is automatically live and running, appears on the **Tasks** list.
...
You can use the same steps outlined earlier to create separate automation tasks for starting the Azure Database for PostgreSQL flexible server at a specific time. Here's how:

1. Follow the steps outlined in the "Create an automation task" section until you reach the "Select a template" stage.
1. Instead of selecting the **Stop PostgreSQL flexible server** task, select the template for **Start PostgreSQL flexible server**.
1. Fill in the rest of the required details as described in the subsequent steps. Define the specific schedule at which you want the server to start in the **Configure** section.
...
Here are the possible statuses for a run:
 ...
 | **Waiting** | The run hasn't started yet and is paused because an earlier task is still running. |

 For more information, see [Review runs history in monitoring view](/azure/logic-apps/monitor-logic-apps#review-runs-history).

## Edit the task

To change a task, use the following options:
...
- For details on editing the underlying workflow, see [Edit the task's underlying workflow](/azure/logic-apps/create-automation-tasks-azure-resources#edit-the-tasks-underlying-workflow).
