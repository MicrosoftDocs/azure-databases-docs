---
title: Delete Virtual Endpoints in Azure Database for PostgreSQL Flexible Server
description: This article describes how to delete virtual endpoints for an Azure Database for PostgreSQL flexible server.
#customer intent: As a user, I want to delete virtual endpoints in Azure Database for PostgreSQL flexible server, so that I can remove connections I no longer need.
author: gkasar
ms.author: gkasar
ms.reviewer: maghan
ms.date: 07/13/2026
...
# Delete virtual endpoints in Azure Database for PostgreSQL flexible server

This article provides step-by-step instructions to delete virtual endpoints associated with an Azure Database for PostgreSQL flexible server.
...
1.  In the **Virtual endpoints** section, select the ellipsis to the right of the pencil icon, and then select **Delete**.
...
Use the [`az postgres flexible-server virtual-endpoint delete`](/cli/azure/postgres/flexible-server/replica#az-postgres-flexible-server-virtual-endpoint-delete) command to delete the existing virtual endpoints of your Azure PostgreSQL flexible server.
