---
title: Switch Over Read Replica to Primary in Azure Database for PostgreSQL Flexible Server
description: This article describes how to switch over a read replica so that it becomes the primary.
#customer intent: As a user, I want to switch over a read replica to primary so that I can promote a replica to become the new primary server in my replication set.
author: gkasar
ms.author: gkasar
ms.reviewer: maghan
ms.date: 07/13/2026
...
# Switch over read replica to primary in Azure Database for PostgreSQL flexible server
...
1. If the server doesn't have the virtual endpoints created, or if the read replica that you're trying to switch over to primary isn't the reader virtual endpoint's target server, an attempt to switch over a read replica to primary fails. The warning displayed in the dialog reminds you of this condition. 
...
1. In **Replica server to promote**, select **Promote to primary server** for **Action**. Select **Planned** or **Forced** for **Data sync**, depending on what suits your needs best. If you decide to use the **Forced** option, mark the **I understand that any data changes that aren't replicated from the primary server will be lost. The read replica lag time is the approximate period of data loss.** checkbox to acknowledge the potential data loss. Finally, select **Promote**.
...
Use the [`az postgres flexible-server replica promote`](/cli/azure/postgres/flexible-server/replica#az-postgres-flexible-server-replica-promote) command to switch over a read replica to become the new primary of your Azure PostgreSQL flexible server.
