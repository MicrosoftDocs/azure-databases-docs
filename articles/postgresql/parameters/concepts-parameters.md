---
title: Parameters in Azure Database for PostgreSQL Flexible Server
description: Learn about the parameters for an Azure Database for PostgreSQL flexible server.
#customer intent: As a user, I want to learn about the configurable parameters in Azure Database for PostgreSQL flexible server, so that I can tune my server to meet my workload needs.
author: jasomaning
...
- **Non-database engine**: parameters that control some built-in functionality, which is core to the Azure Database for PostgreSQL service, but isn't part of the database engine or any of its extensions. Some examples of these parameters are `metrics.collector_database_activity` (controls whether the service should collect the list of metrics which are considered [enhanced metrics](../monitor/concepts-monitoring.md#enhanced-metrics) and aren't collected by default), `pgbouncer.enabled` (activates the instance of [PgBouncer](../connectivity/concepts-pgbouncer.md) which is built into the service), and `index_tuning.analysis_interval` (sets the frequency at which [autonomous tuning](../monitor/concepts-autonomous-tuning.md) should wake up to produce recommendations).

## Customize parameters

You can configure both **database engine** and **non-database engine** parameters at the server level. For more information, see [set the value of one or more parameters](how-to-parameters-set-value.md).

> [!NOTE]
> Because Azure Database for PostgreSQL is a managed database service, you don't have host or operating system access to view or modify configuration files such as *postgresql.conf*. The service automatically updates the content of the files based on parameter changes that you make.
...
- [Configure parameters](how-to-parameters-list-all.md).
