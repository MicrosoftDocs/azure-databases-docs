...
description: max_wal_senders parameter for Azure Database for PostgreSQL flexible server.
...
The default value for the `max_wal_senders` parameter set when you provision the instance of Azure Database for PostgreSQL flexible server must never be decreased below `2 (if HA is enabled) + number of read replicas provisioned + slots_used_in_logical_replication`.
