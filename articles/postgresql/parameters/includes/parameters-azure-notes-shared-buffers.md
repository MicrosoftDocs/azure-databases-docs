...
description: shared_buffers parameter for Azure Database for PostgreSQL flexible server.
...
The default value for the `shared_buffers` parameter is calculated when you provision the instance of Azure Database for PostgreSQL flexible server, based on the product name that you select for its compute. Any subsequent changes of product selection to the compute that supports the flexible server don't have any effect on the default value for the `shared_buffers` parameter of that instance.
