...
Network administrators create the cloud networking infrastructure, such as Azure Private Link services. Database administrators create and manage the database servers, such as Azure Database for PostgreSQL flexible server.
...
Use the [az network private-endpoint-connection reject](/cli/azure/network/private-endpoint-connection#az-network-private-endpoint-connection-reject) command to reject one private endpoint connection that's in a pending state.
