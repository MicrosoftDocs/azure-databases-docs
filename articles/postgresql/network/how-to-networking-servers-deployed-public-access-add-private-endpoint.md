...
1. If you have the required permissions to deploy a private endpoint, select **Create private endpoint** to create it.
...
1. In the **Basics** page, enter all the required details. Then, select **Next: Resource**.
...
1. Use the following table to understand the meaning of the different fields available in the **Basics** page, and as guidance to fill the page:
    ...
    | --- | --- | --- |
    | **Subscription** | Select the name of the [subscription](/microsoft-365/enterprise/subscriptions-licenses-accounts-and-tenants-for-microsoft-cloud-offerings#subscriptions) in which you want to create the resource. The portal automatically selects the subscription in which your server is deployed. | A subscription is an agreement with Microsoft to use one or more Microsoft cloud platforms or services. Charges accrue based on either a per-user license fee or on cloud-based resource consumption. If you have multiple subscriptions, choose the subscription in which you want to be billed for the resource. |
    | **Resource group** | The [resource group](/azure/azure-resource-manager/management/manage-resource-groups-portal#what-is-a-resource-group) in the selected subscription, in which you want to create the private endpoint. It can be an existing resource group, or you can select **Create new**, and provide a name in that subscription which is unique among the existing resource group names. The portal automatically selects the resource group in which your server is deployed. | A resource group is a container that holds related resources for an Azure solution. The resource group can include all the resources for the solution, or only those resources that you want to manage as a group. You decide how you want to allocate resources to resource groups based on what makes the most sense for your organization. Generally, add resources that share the same lifecycle to the same resource group so you can easily deploy, update, and delete them as a group. |
    | **Name** | The name that you want to assign to the private endpoint. | A unique name that identifies the private endpoint through which you connect to your Azure Database for PostgreSQL flexible server. |
...
1. In the **Resource** page, enter all the required details. Then, select **Next: Virtual Network**.
...
1. Use the following table to understand the meaning of the different fields available in the **Resource** page, and as guidance to fill the page:

    | Setting | Suggested value | Description |
    | --- | --- | --- |
    | **Resource type** | Automatically set to `Microsoft.DBforPostgreSQL/flexibleServers` | This value is automatically chosen for you. It corresponds to the type of resource that an Azure Database for PostgreSQL flexible server is, to the eyes of Azure Private Link. |
    | **Resource** | Automatically set to the name of the Azure Database for PostgreSQL flexible server for which you're creating the private endpoint. | The name of the resource to which the private endpoint connects. |
    | **Target sub-resource** | Automatically set to `postgresqlServer`. | The type of subresource for the resource selected, that your private endpoint can access. |

1. In the **Virtual Network** page, enter all the required details. Then, select **Next: DNS**.
...
1. Use the following table to understand the meaning of the different fields available in the **Virtual Network** page, and as guidance to fill the page:
...
1. In the **DNS** page, enter all the required details. Then, select **Next: Tags**.
...
1. Use the following table to understand the meaning of the different fields available in the **DNS** page, and as guidance to fill the page:
    ...
    | **Subscription** | Select the name of the [subscription](/microsoft-365/enterprise/subscriptions-licenses-accounts-and-tenants-for-microsoft-cloud-offerings#subscriptions) in which you want to create the private DNS zone. The portal automaticallyselects the subscription in which your server is deployed. | A subscription is an agreement with Microsoft to use one or more Microsoft cloud platforms or services, for which charges accrue based on either a per-user license fee or on cloud-based resource consumption. If you have multiple subscriptions, choose the subscription in which you'd like to be billed for the resource. |
    | **Resource group** | The [resource group](/azure/azure-resource-manager/management/manage-resource-groups-portal#what-is-a-resource-group) in the selected subscription, in which you want to create the private DNS zone. It must be an existing resource group. The portal automaticallyselects the resource group in which your server is deployed. | A resource group is a container that holds related resources for an Azure solution. The resource group can include all the resources for the solution, or only those resources that you want to manage as a group. You decide how you want to allocate resources to resource groups based on what makes the most sense for your organization. Generally, add resources that share the same lifecycle to the same resource group so you can easily deploy, update, and delete them as a group. |
...
1. In the **Tags** page, fill all the details required. Then, select **Next: Review + create**.
...
1. Use the following table to understand the meaning of the different fields available in the **Tags** page, and as guidance to fill the page:
...
1. In the **Review + create** page, make sure that everything is configured as you wanted to. Then, select **Create**.
...
1.  A new deployment is launched to create your private endpoint.

    :::image type="content" source="./media/how-to-networking/create-private-endpoint-deployment-progress.png" alt-text="Screenshot showing the in progress deployment of the private endpoint." lightbox="./media/how-to-networking/create-private-endpoint-deployment-succeeded.png":::

1. When the deployment completes, you can select **Go to resource**, to take you to the just created private endpoint.
...
Use the [az network private-endpoint create](/cli/azure/network/private-endpoint#az-network-private-endpoint-create) command to create the private endpoint connection. You must have the required permissions to deploy a private endpoint and to approve the private endpoint connection for your server.
...
If you have the required permissions, the private endpoint connection is automatically approved. In this case, the output of the following command shows `status` as `Approved`, and `description` as `Auto-Approved`:
...
The `az network private-endpoint create` command creates a `privatelink.postgres.database.azure.com` private DNS zone, if it doesn't exist. It also links the private DNS zone to the virtual network in which you create the private endpoint. However, it doesn't create a DNS zone group in the private endpoint. If you want, you can integrate your private endpoint with a private DNS zone. To do so, 
...
If you try to create the private endpoint with a statically allocated private IP address, and the address you specified is already used by some other network interface, you get the following error:
