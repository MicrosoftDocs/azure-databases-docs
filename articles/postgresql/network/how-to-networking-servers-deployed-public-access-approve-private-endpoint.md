...
1. Select the private endpoint connection whose state is **Pending** that you want to approve. Select **Approve** to start the approval process for the private endpoint connection.
...
1. If necessary, in the **Description** box of the **Approve** dialog, type the reason why you're approving this private endpoint connection, and select **Yes**.
...
1. When the operation finishes, a notification informs you that the private endpoint connection was successfully approved.
...
Use the [az network private-endpoint-connection approve](/cli/azure/network/private-endpoint-connection#az-network-private-endpoint-connection-approve) command to approve one private endpoint connection that's in pending state.
