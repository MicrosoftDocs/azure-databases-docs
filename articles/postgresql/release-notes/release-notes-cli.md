...
| Commands | Comments |
| --- | --- |
| `az postgres flexible-server update` | Fix bug where geo-backup data encryption properties were not updated |
| `az postgres flexible-server fabric-mirroring` | Fix space-separated list handling for `start` and `update-databases` |
| `az postgres flexible-server create` | Support adding admin during create when `--active-directory-auth` is enabled, and do not generate a password when `--password-auth` is disabled |
...
| Commands | Comments |
| --- | --- |
| `az postgres flexible-server restore` | Add support to restore to a new server using `PremiumV2_LRS` by setting storage type |
