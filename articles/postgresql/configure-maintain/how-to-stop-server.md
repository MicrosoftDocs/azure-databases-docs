...
1. A notification informs you that the server is stopping. Also, the status of the server changes to **Stopping**.
...
1. When the process completes, a notification informs you that the server is stopped. Also, the status of the server changes to **Stopped**.
...
If you attempt to stop the compute of a server which isn't in `Ready` state and isn't processing any other operation, you receive an error like this:
...
If you attempt to stop the compute of a server which is processing any other operation, you receive an error like this:
...
> While the Azure Database for PostgreSQL flexible server is in stopped state, it could be briefly restarted for scheduled monthly maintenance, and then returned to its stopped state. This procedure ensures that even flexible servers in a stopped state, stay up to date with all necessary patches and updates.
