...
description: This article provides considerations and guidelines for configuring and managing Azure Database for PostgreSQL flexible servers.
#customer intent: As a user new to Azure, I want to understand what an Azure Database for PostgreSQL flexible server is, so that I can determine how it maps to my on-premises PostgreSQL knowledge.
author: kabharati
ms.author: kabharati
...
This article provides considerations and guidelines for working with an Azure Database for PostgreSQL flexible server.

## What is an Azure Database for PostgreSQL server?

A server in the Azure Database for PostgreSQL flexible server option is a central administrative point for multiple databases. It's the same PostgreSQL server construct that you might be familiar with in the on-premises world. Specifically, an Azure Database for PostgreSQL flexible server is managed, provides performance guarantees, and exposes access and features at the server level.
...
| :-- | :-- |
| Authentication and authorization | An Azure Database for PostgreSQL flexible server supports native PostgreSQL authentication. You can connect and authenticate to a server by using the server's admin login. |
...
You can manage Azure Database for PostgreSQL flexible servers by using the [Azure portal](https://portal.azure.com) or the [Azure CLI](/cli/azure/postgres).
...
The Azure Database for PostgreSQL flexible server parameters determine the configuration of the server. In an Azure Database for PostgreSQL flexible server, you can view and edit the list of parameters by using the Azure portal or the Azure CLI.
...
Your Azure Database for PostgreSQL flexible server is enabled with default values for each parameter on creation. You can't configure some parameters that require a server restart or superuser access for changes to take effect.
