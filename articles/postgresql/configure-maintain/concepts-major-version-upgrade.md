...
Azure Database for PostgreSQL flexible server provides Pre-Upgrade Validation Checks (PVC) to help assess upgrade readiness before starting a major version upgrade.

Pre-Upgrade Validation Checks run a series of compatibility and configuration validations against the server to identify conditions that could cause the upgrade to fail or behave unexpectedly. Common checks include unsupported extensions, logical replication slots, prepared transactions, event triggers, unsupported object dependencies, and pending restart-required configuration changes.
...
- **No blocking issues detected**: Pre-Upgrade Validation Checks completed successfully and didn't identify any issues that would block the upgrade.
- **Blocking issues detected**: Pre-Upgrade Validation Checks identified one or more issues that must be resolved before the upgrade can proceed.

Depending on the results, you can either proceed with the upgrade or remediate the reported issues and rerun validation.

### Limitations

When using Pre-Upgrade Validation Checks, consider the following limitations:
...
For step-by-step instructions, see [How to run Pre-Upgrade Validation Checks](how-to-run-upgrade-validation-checks.md).
...
- [Read replicas](../read-replica/concepts-read-replicas-geo.md) aren't supported during in-place upgrades. You must delete the read replica (including any cascading read replica) before upgrading the primary server. After the upgrade, you can re-create the replica.
...
- When upgrading to PostgreSQL 17 and above, the following extensions are **not supported** and must be removed before upgrade. You can re-enable them only if supported on the target version: `age`, `azure_ai`, `hll`, `pg_diskann`, `pgrouting`.
...
- [Pre-Upgrade Validation Checks](./how-to-run-upgrade-validation-checks.md).
