...
description: Introducing support for geospatial queries on Azure DocumentDB.
...
Geospatial data can now be stored and queried using Azure DocumentDB. This enhancement provides powerful tools to manage and analyze spatial data, enabling a wide range of applications such as real-time location tracking, route optimization, and spatial analytics.
...
* Currently, querying with a single-ringed GeoJSON polygon whose area exceeds a single hemisphere isn't supported. In such cases, Azure DocumentDB returns the following error message:
   ```json
...
- Read more about [MongoDB Query Language (MQL) compatibility with MongoDB.](compatibility-query-language.md)
- Review options for [migrating from MongoDB to Azure DocumentDB.](how-to-migrate-native-tools.md)
