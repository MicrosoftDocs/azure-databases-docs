---
  ...
  description: Review a series of practical indexing examples illustrating indexing at various nesting levels in Azure DocumentDB.
  author: avijitgupta
...
# Scenario based guide to indexing in Azure DocumentDB
...
Azure DocumentDB allows indexes on root properties. The example allows searching `sampleColl` by `car_id`.
...
Azure DocumentDB allows indexing embedded document properties. The example creates an index  on field `registration_datetime` within a nested document `registration`.
...
Azure DocumentDB allows indexing the root property defined as an array. Let us consider following json sample.
...
Azure DocumentDB allows indexing nested arrays. The example creates an index on `resolutions` field existing within `complains` array.
...
Azure DocumentDB allows indexing fields within an array. The example creates an index on `date` field within `accidents` array.
...
Azure DocumentDB supports Wildcard indexes. The example allows us to exclude indexing all nested fields within document `car_info`.
...
Azure DocumentDB supports Wildcard indexes. The example allows us to exclude nested objects from the document.
