...
description: Configure and use text indexes to perform and fine tune text searches in Azure DocumentDB.
author: gahl-levy
ms.author: gahllevy
ms.topic: how-to
ms.date: 08/28/2023
...
# Search and query with text indexes in Azure DocumentDB

One of the key features that Azure DocumentDB provides is text indexing, which allows for efficient searching and querying of text-based data. The service implements **version 2** text indexes. Version 2 supports case sensitivity but not diacritic sensitivity.

Text indexes in Azure DocumentDB are special data structures that optimize text-based queries, making them faster and more efficient. They're designed to handle textual content like documents, articles, comments, or any other text-heavy data. Text indexes use techniques such as tokenization, stemming, and stop words to create an index that improves the performance of text-based searches.
...
1. Use the `createIndex` method with the `text` option to create a text index on the `title` field.
    ...
    > [!NOTE]
    > While you can define only one text index per collection, Azure DocumentDB allows you to create text indexes on multiple fields to enable you to perform text searches across different fields in your documents.
...
When creating a text index, you can assign different weights to individual fields in the index. These weights represent the importance or relevance of each field in the search. Azure DocumentDB calculates a score and assigned weights for each document based on the search terms when executing a text search query. The score represents the relevance of the document to the search query.
...
1. Perform a text search for the phrase `DocumentDB`.
