...
description: Enhance AI-based applications using retrieval-augmented generation (RAG) with Azure DocumentDB, LangChain, and OpenAI.
author: khelanmodi
...
# Retrieval-augmented generation (RAG) with Azure DocumentDB
...
- **Create the following resources on Microsoft Azure:**
  - **Azure DocumentDB cluster**: For more information, see [create a cluster](quickstart-portal.md)
...
1. Set the Azure DocumentDB connection string, database name, collection name, and index:
...
1. Create embeddings from the data, save to the database, and return a connection to your vector store, Azure DocumentDB.
...
1. Create the following [HNSW vector index](./vector-search.md) on the collection. (Note that the name of the index is the same.)
...
1. Define a function that performs semantic similarity search using Azure DocumentDB Vector Search on a query. (Note that this code snippet is just a test function.)
